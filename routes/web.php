...
Route::middleware('auth')->group(function () {
    ...
    Route::middleware(['app.active', 'active.Subscription'])->group(function () {
        ...
        // 5. الطرود المستلمة (محمية بـ Shipment_In)
        ...
        Route::post('/passengers/{id}/status', [PassengersController::class, 'updateStatus'])->name('passengers.updateStatus');
        ...
        // 7. حزم الترحيل المستلمة (محمية بـ Package_In)
        ...
        Route::post('/shipment/{id}/status', [ShipmentController::class, 'updateStatus'])->name('shipment.updateStatus');
        Route::patch('/shipment/{id}/return', [ShipmentController::class, 'returnShipment'])->name('shipment.return');
        Route::patch('/shipment/{id}/cancel', [ShipmentController::class, 'cancelShipment'])->name('shipment.cancel');

        Route::put('shipment/updatePaymentMethod/{id}', [ShipmentController::class, 'updatePaymentMethod'])->name('shipment.updatePaymentMethod');
        Route::get('/shipments/{id}/thermal', [InvoiceController::class, 'printThermal'])->name('shipment.printThermal');
        Route::post('/users/toggle-status/{id}', [UserController::class, 'toggleStatus']);

        Route::get('/whatsapp/sender/{id}', [WhatsAppController::class, 'openForSender'])->name('whatsapp.sender');
        Route::get('/whatsapp/receiver/{id}', [WhatsAppController::class, 'openForReceiver'])->name('whatsapp.receiver');
        Route::get('/whatsapp/branch-manifest/{id}/{branchCode?}', [WhatsAppController::class, 'sendBranchManifest'])->name('whatsapp.branchManifest');
        ...
        Route::get('/drivers/{id}/shipments/print', [DriverController::class, 'printShipments'])->name('drivers.shipments.print')->middleware('check.service:Drivers');
        ...
        Route::get('/shipments/select-customer', [ShipmentController::class, 'selectCustomer'])->name('shipments.selectCustomer');
        Route::get('/shipments/create-customer', [ShipmentController::class, 'createCustomer'])->name('shipments.createCustomer');
        Route::post('/shipments/store-customer', [ShipmentController::class, 'storeCustomer'])->name('shipments.storeCustomer');

        Route::get('/customers/search', [CustomerController::class, 'search'])->name('customers.search');
        Route::get('/customers/{id}/comprehensive-report', [CustomerController::class, 'comprehensiveReport'])->name('customers.comprehensive-report');
        Route::post('/customers/{customer}/clear-balance', [CustomerController::class, 'clearBalance'])->name('customers.clear-balance');

        // دفعات العملاء
        Route::post('/shipments/{shipment}/payment', [CustomerPaymentController::class, 'store'])->name('customer-payments.store');
        Route::get('/shipments/{shipment}/payments', [CustomerPaymentController::class, 'index'])->name('customer-payments.index');
        ...
        Route::get('/finance/settlements/create', [BranchFinanceController::class, 'createSettlement'])->name('finance.settlements.create');
        Route::post('/finance/settlements', [FinanceSettlementController::class, 'store'])->name('finance.settlements.store');
        Route::get('/api/branches/{branch}/balance', [BranchFinanceController::class, 'apiBranchBalance'])->name('api.branch.balance');

        // التقارير
        Route::get('/reports', [ReportController::class, 'index'])->name('reports.index');
        Route::get('/reports/dashboard', [ReportController::class, 'dashboard'])->name('reports.dashboard');
        Route::get('/reports/shipments', [ReportController::class, 'shipments'])->name('reports.shipments');
        Route::get('/reports/customers/{id}', [ReportController::class, 'customerStatement'])->name('reports.customers.statement');
        Route::get('/reports/customers/{id}/pdf', [ReportController::class, 'customerStatementPDF'])->name('reports.customers.statement.pdf');
        Route::get('/reports/branches/{id}', [ReportController::class, 'branchStatement'])->name('reports.branches.statement');
        Route::get('/reports/branches/{id}/pdf', [ReportController::class, 'branchStatementPDF'])->name('reports.branches.statement.pdf');
        Route::get('/reports/monthly/closing/pdf', [ReportController::class, 'monthlyClosingPDF'])->name('reports.monthly.closing.pdf');
        Route::get('/reports/revenue', [ReportController::class, 'dashboard'])->name('reports.revenue');
        ...
        // دفعات الحزم للفروع
        Route::post('/branch-package-payment', [App\Http\Controllers\BranchPackagePaymentController::class, 'store'])->name('branch-package-payment.store');
        Route::get('/branch-package-payment/{branchShipmentPackageId}', [App\Http\Controllers\BranchPackagePaymentController::class, 'index'])->name('branch-package-payment.index');
        Route::delete('/branch-package-payment/{id}', [App\Http\Controllers\BranchPackagePaymentController::class, 'destroy'])->name('branch-package-payment.destroy');
        ...
        Route::patch('/shipment-packages/{id}/mark-all-delivered', [ShipmentPackagesController::class, 'markAllDelivered'])->name('shipmentpackage.mark-all-delivered');
        Route::patch('/shipment-packages/{shipmentId}/unlink', [ShipmentPackagesController::class, 'unlinkFromPackage'])->name('shipmentpackage.unlink');
