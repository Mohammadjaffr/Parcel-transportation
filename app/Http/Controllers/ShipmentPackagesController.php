    ...
    public function incomingStore(Request $request)
{
    ...
    // 2. التحقق من فشل المدخلات وإرجاع الخطأ المخصص
    if ($validator->fails()) {
        ...
        return back()
            ...
            ->withInput()
            ->with('error', 'يرجى مراجعة الحقول؛ الأخطاء الحالية: ' . $detailedErrors);
    }
   ...
   public function incomingShow(Request $request, $id)
    ...
    public function printManifest($id)
    {
        ...
        return view('pages.shipmentpackage.manifest_pdf', compact('package'));
    }
    public function printManifestD($id)
    {
        ...
        return view('pages.shipmentpackage.manifest_driver_pdf', compact('package'));
    }
