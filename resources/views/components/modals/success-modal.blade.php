...
<div x-data="{ isSuccessModalOpen: @if(session('success')) true @else false @endif }">
    ...
    <div x-show="isSuccessModalOpen" style="display: none;" 
        ...
        <div class="fixed inset-0 w-full h-full backdrop-blur-sm bg-slate-900/40" @click="isSuccessModalOpen = false"></div>

        <!-- صندوق المودل (Responsive Box) -->
        <!-- استخدام w-full للجوال، و sm:max-w-md للكمبيوتر ليثبت عند حجم معين -->
        <div x-show="isSuccessModalOpen"
            ...
            <div class="text-center">
                <!-- الأيقونة (تصغر قليلاً في الجوال وتكبر في الكمبيوتر) -->
                <div class="flex relative z-10 justify-center items-center mb-6">
                    <svg class="text-emerald-100 w-[80px] h-[80px] sm:w-[90px] sm:h-[90px]" viewBox="0 0 90 90" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                        <svg class="w-8 h-8 text-emerald-500 sm:w-10 sm:h-10" viewBox="0 0 38 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M5.9375 19.0004C5.9375 11.7854 11.7864 5.93652 19.0014 5.93652C26.2164 5.93652 32.0653 11.7854 32.0653 19.0004C32.0653 26.2154 26.2164 32.0643 19.0014 32.0643C11.7864 32.0643 5.9375 26.2154 5.9375 19.0004ZM19.0014 2.93652C10.1296 2.93652 2.9375 10.1286 2.9375 19.0004C2.9375 27.8723 10.1296 35.0643 19.0014 35.0643C27.8733 35.0643 35.0653 27.8723 35.0653 19.0004C35.0653 10.1286 27.8733 2.93652 19.0014 2.93652ZM24.7855 17.0575C25.3713 16.4717 25.3713 15.522 24.7855 14.9362C24.1997 14.3504 23.25 14.3504 22.6642 14.9362L17.7177 19.8827L15.3387 17.5037C14.7529 16.9179 13.8031 16.9179 13.2173 17.5037C12.6316 18.0894 12.6316 19.0392 13.2173 19.625L16.657 23.0647C16.9383 23.346 17.3199 23.504 17.7177 23.504C18.1155 23.504 18.4971 23.346 18.7784 23.0647L24.7855 17.0575Z" fill="currentColor" />
                        </svg>
                    </span>
                </div>
                ...
                <div class="flex flex-col gap-3 mt-8 sm:mt-10">
                    <button @click="isSuccessModalOpen = false" type="button"
                        class="w-full sm:w-auto sm:min-w-[140px] sm:mx-auto py-3.5 sm:py-3 px-6 text-sm sm:text-base font-bold font-headline text-white rounded-2xl bg-emerald-500 hover:bg-emerald-600 active:scale-95 transition-all shadow-lg shadow-emerald-500/20">
                        {{ session('success_buttonText') ?? 'حسناً' }}
                    </button>
                </div>
            </div>
