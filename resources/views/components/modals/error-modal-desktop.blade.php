...
<div x-data="{ isErrorModalOpen: @if (session('error')) true @else false @endif }">
    ...
    <template x-teleport="body">
        <div x-cloak x-show="isErrorModalOpen" 
            ...
            <div x-show="isErrorModalOpen" 
                ...
                <div class="flex justify-center items-center mb-6">
                    <div class="flex relative justify-center items-center w-24 h-24 bg-red-50 rounded-full dark:bg-red-500/10">
                        {{-- الخلفية الزخرفية الدوارة (حمراء) --}}
                        <svg class="absolute text-red-100 dark:text-red-500/20 w-[90px] h-[90px] animate-[spin_10s_linear_infinite]" viewBox="0 0 90 90" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        {{-- أيقونة الخطأ (X) --}}
                        <svg class="relative z-10 w-12 h-12 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button @click="isErrorModalOpen = false" type="button"
                        class="py-3.5 w-full text-sm font-bold text-white bg-red-500 rounded-xl transition-all hover:bg-red-600 hover:shadow-lg hover:shadow-red-500/30 active:scale-95 focus:outline-none">
                    {{ session('error_buttonText') ?? 'حسناً' }}
                </button>
            </div>
        </div>
    </template>
