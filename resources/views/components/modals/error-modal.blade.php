{{-- <div x-data="{ isErrorModalOpen: @if(session('error')) true @else false @endif }">
    <div x-show="isErrorModalOpen" class="flex fixed inset-0 justify-center items-center p-5 z-99999">
        ...
        <div class="relative p-6 w-full max-w-md bg-white rounded-2xl shadow-xl dark:bg-gray-900">
            <div class="py-4 text-center">
                ...
                <p class="text-sm leading-6 text-gray-500 dark:text-gray-400">
                    {{session('error_message')  ?? 'حدث خطأ ما.' }}
                </p>
...
<div x-data="{ isErrorModalOpen: @if(session('error')) true @else false @endif }">
    <div x-show="isErrorModalOpen" style="display: none;" 
        ...
        <div class="fixed inset-0 w-full h-full backdrop-blur-sm bg-slate-900/40" @click="isErrorModalOpen = false"></div>
        ...
        <div x-show="isErrorModalOpen"
            ...
            <div class="text-center">
                <div class="flex relative z-10 justify-center items-center mb-6">
                    <svg class="text-rose-100 w-[80px] h-[80px] sm:w-[90px] sm:h-[90px]" viewBox="0 0 90 90" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <span class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                        <svg class="w-8 h-8 text-rose-500 sm:w-10 sm:h-10" viewBox="0 0 38 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd" d="M9.62684 11.7496C9.04105 11.1638 9.04105 10.2141 9.62684 9.6283C10.2126 9.04252 11.1624 9.04252 11.7482 9.6283L18.9985 16.8786L26.2485 9.62851C26.8343 9.04273 27.7841 9.04273 28.3699 9.62851C28.9556 10.2143 28.9556 11.164 28.3699 11.7498L21.1198 18.9999L28.3699 26.25C28.9556 26.8358 28.9556 27.7855 28.3699 28.3713C27.7841 28.9571 26.8343 28.9571 26.2485 28.3713L18.9985 21.1212L11.7482 28.3715C11.1624 28.9573 10.2126 28.9573 9.62684 28.3715C9.04105 27.7857 9.04105 26.836 9.62684 26.2502L16.8771 18.9999L9.62684 11.7496Z" fill="currentColor" />
                        </svg>
                    </span>
                </div>
                ...
                <p class="px-2 text-sm font-medium leading-relaxed sm:text-base text-slate-500 sm:px-4">
                    {{ session('error_message') ?? 'حدث خطأ ما يرجى المحاولة مرة أخرى.' }}
                </p>
