...
<div class="max-w-4xl w-full mx-auto bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] print-no-shadow overflow-hidden border border-slate-100 print-border my-8 print:my-0">
    ...
    <div class="bg-slate-50 p-6 sm:p-8 border-b border-slate-100">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
            ...
            <div class="text-right">
                <div class="inline-flex items-center justify-center px-4 py-2 bg-brand-50 text-brand-700 rounded-xl font-bold text-sm mb-2 border border-brand-100">
                    {{ $title ?? 'كشف حساب عميل' }}
                </div>
    ...
    <div class="p-6 sm:p-8">
        ...
        <div class="mb-8">
            ...
            <div class="rounded-2xl border border-slate-200 overflow-hidden">
                            ...
                            <tr class="transition-colors hover:bg-slate-50">
                                ...
                                <td class="font-bold text-slate-700">
                                    @if(in_array($movementType, ['shipment', 'شحنة', 'قيد مديونية']))
                                        <span class="inline-flex items-center px-2 py-1 rounded bg-blue-50 text-blue-600 text-xs">{{ $movementType }}</span>
                                    @elseif(in_array($movementType, ['payment', 'دفعة', 'سداد / تحصيل']))
                                        <span class="inline-flex items-center px-2 py-1 rounded bg-emerald-50 text-emerald-600 text-xs">{{ $movementType }}</span>
                                    @else
                                        <span class="inline-flex items-center px-2 py-1 rounded bg-slate-50 text-slate-600 text-xs">{{ $movementType }}</span>
                                    @endif
                                </td>
                                <td>
                                    <div class="text-slate-800 font-bold">{{ $entry['description'] ?? '---' }}</div>
                                    @if(!empty($entry['reference']) && $entry['reference'] !== '---')
                                        <div class="text-xs text-slate-400 mt-0.5">مرجع: <span dir="ltr">{{ $entry['reference'] }}</span></div>
                                    @endif
                                    @if(!empty($entry['notes']))
                                        <div class="text-xs text-slate-400 mt-0.5">{{ $entry['notes'] }}</div>
                                    @endif
                                </td>
                                <td class="text-center font-black {{ $isDebitEntry ? 'text-rose-600' : 'text-emerald-600' }}" dir="ltr">
                ...
                <!-- Financial Totals -->
                <div class="bg-slate-50 border-t border-slate-200 p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    <div class="flex items-center gap-2 bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm">
                        ...
                        <span class="{{ $sumIsDebtor ? 'text-rose-600' : 'text-emerald-600' }} font-black text-xl" dir="ltr">
                            {{ $sumFinalBalance }} ر.ي
                        </span>
                        @if(isset($summary['balance_status']))
                        <span class="text-xs font-bold {{ $sumIsDebtor ? 'text-rose-500' : 'text-emerald-500' }} bg-{{ $sumIsDebtor ? 'rose' : 'emerald' }}-50 px-2 py-0.5 rounded-lg">
        ...
        <div class="mb-8">
            ...
            <div class="rounded-2xl border border-slate-200 overflow-hidden">
                        ...
                        @forelse($shipmentRows as $shipment)
                            <tr class="transition-colors hover:bg-slate-50">
                                ...
                                <td class="text-center">
                                    ...
                                    @endphp
                                    <span class="inline-flex items-center px-2 py-1 rounded-lg text-xs font-bold {{ $dirColor }}">
                                        {{ $dirLabel }}
                                    </span>
                                </td>
                                <td class="text-center text-slate-600 font-bold text-xs">{{ $shipment['payment_method'] ?? '---' }}</td>
                                <td class="text-center font-black text-slate-800" dir="ltr">
                                    {{ is_numeric($shipment['total_amount'] ?? null) ? number_format((float)$shipment['total_amount'], 0) : ($shipment['total_amount'] ?? '0') }} ر.ي
                                </td>
                            </tr>
                        @empty
                            <tr>
                ...
                <!-- Shipment Stats -->
                <div class="bg-slate-50 border-t border-slate-200 p-4 grid grid-cols-3 gap-4 text-center divide-x divide-x-reverse divide-slate-200">
                    <div>
    ...
    <div class="bg-slate-800 text-slate-400 p-4 text-center text-xs font-medium rounded-b-[2rem]">
        تم الإنشاء إلكترونياً عبر نظام {{ $companyName }} | بواسطة: {{ $creator_name ?? (auth()->user()->name ?? 'مسؤول النظام') }}
    </div>
</div>
@endsection