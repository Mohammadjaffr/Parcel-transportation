...
@section('content')
    <div
        ...
        <!-- Header -->
        <div class="bg-gradient-to-l from-orange-50/50 to-white p-6 sm:p-8 border-b border-slate-100">
        ...
        <div class="p-6 sm:p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="bg-emerald-50/50 rounded-2xl border border-emerald-100 p-5">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <h3 class="text-xs font-bold text-emerald-600 uppercase tracking-wider">المرسل</h3>
                    </div>
                    <div class="space-y-2">
                        <div><span class="text-xs text-slate-400">الاسم:</span> <span
                                class="text-slate-800 font-bold text-sm">{{ $sender_name ?? '---' }}</span></div>
                        <div><span class="text-xs text-slate-400">الهاتف:</span> <span
                                class="text-slate-800 font-bold text-sm" dir="ltr">{{ $sender_phone ?? '---' }}</span></div>
                        <div><span class="text-xs text-slate-400">الفرع:</span> <span
                                class="text-slate-700 font-medium text-sm">{{ $sender_branch ?? '---' }}</span></div>
                        <div><span class="text-xs text-slate-400">المكتب:</span> <span
                                class="text-slate-700 font-medium text-sm">{{ $sender_office ?? '---' }}</span></div>
                    </div>
                </div>
                ...
                <div class="bg-blue-50/50 rounded-2xl border border-blue-100 p-5">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <h3 class="text-xs font-bold text-blue-600 uppercase tracking-wider">المستلم</h3>
                    </div>
                    <div class="space-y-2">
                        <div><span class="text-xs text-slate-400">الاسم:</span> <span
                                class="text-slate-800 font-bold text-sm">{{ $receiver_name ?? '---' }}</span></div>
                        <div><span class="text-xs text-slate-400">الهاتف:</span> <span
                                class="text-slate-800 font-bold text-sm" dir="ltr">{{ $receiver_phone ?? '---' }}</span>
                        </div>
                        <div><span class="text-xs text-slate-400">الفرع:</span> <span
                                class="text-slate-700 font-medium text-sm">{{ $receiver_branch ?? '---' }}</span></div>
                        <div><span class="text-xs text-slate-400">المكتب:</span> <span
                                class="text-slate-700 font-medium text-sm">{{ $receiver_office ?? '---' }}</span></div>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-8 mt-10 mb-2">
                <div class="text-center">
                    <p class="text-sm font-bold text-slate-500 mb-8">توقيع المرسل</p>
