...
<div class="max-w-2xl w-full mx-auto bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] print-no-shadow overflow-hidden border border-slate-100 print-border my-8 print:my-0">
    ...
    <div class="p-6 sm:p-8 border-b border-slate-100 {{ $is_credit ? 'bg-gradient-to-l from-emerald-50/50 to-white' : 'bg-gradient-to-l from-rose-50/50 to-white' }}">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-5">
            <div class="flex items-center gap-4">
                
                <div class="w-14 h-14 rounded-2xl bg-white shadow-sm flex items-center justify-center p-2 border border-slate-100 shrink-0">
                    @if(!empty($company['logo']))
                        <img src="{{ $company['logo'] }}" alt="Logo" class="w-full h-full object-contain">
                    @else
                        <img src="{{ asset('assets/image/icon_without_bg.png') }}" alt="Logo" class="w-full h-full object-contain">
                    @endif
                </div>
            ...
            <div class="text-right">
                <div class="inline-flex items-center px-3 py-1.5 rounded-xl font-bold text-xs border {{ $is_credit ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-rose-50 text-rose-700 border-rose-100' }}">
                    {{ $title }}
                </div>
    ...
    <div class="p-6 sm:p-8 space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="bg-blue-50/50 rounded-2xl border border-blue-100 p-5">
                ...
                <div class="space-y-2">
                    <div><span class="text-xs text-slate-400">الاسم:</span> <span class="text-slate-800 font-bold text-sm">{{ $customer_name }}</span></div>
                    <div><span class="text-xs text-slate-400">الهاتف:</span> <span class="text-slate-800 font-bold text-sm" dir="ltr">{{ $customer_phone }}</span></div>
                    <div><span class="text-xs text-slate-400">الفرع:</span> <span class="text-slate-700 font-medium text-sm">{{ $customer_branch }}</span></div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 p-5">
                ...
                <div class="space-y-2">
                    <div><span class="text-xs text-slate-400">طريقة الدفع:</span> <span class="text-slate-800 font-bold text-sm">{{ $payment_method }}</span></div>
                    <div><span class="text-xs text-slate-400">المرجع:</span> <span class="text-slate-800 font-bold text-sm">{{ $reference_number }}</span></div>
                    <div><span class="text-xs text-slate-400">التاريخ:</span> <span class="text-slate-700 font-medium text-sm">{{ $date }}</span></div>
                </div>
            </div>
        </div>
