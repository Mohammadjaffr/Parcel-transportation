            ...
            <div class="2xsm:gap-3 flex items-center gap-2">
                ...
                <div x-data="{
                    ...
                    async performBackup() {
                        ...
                        try {
                            ...
                            if (response.ok && data.status) {
                                this.showToast('تم رفع النسخة الاحتياطية بنجاح', 'success');
                            } else {
                                this.showToast(data.message || 'فشل رفع النسخة الاحتياطية', 'error');
                            }
                        } catch (error) {
                            this.showToast('حدث خطأ أثناء رفع النسخة الاحتياطية', 'error');
                        } finally {
                    ...
                    <div x-show="toastVisible" x-transition:enter="transition ease-out duration-300"
                        ...
                        class="fixed top-5 right-5 z-[9999] flex items-center gap-3 rounded-xl px-5 py-3 shadow-lg"
                        ...
                        <svg x-show="toastType === 'success'"
                            class="h-5 w-5 shrink-0 text-green-500 dark:text-green-400" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <!-- Error Icon -->
                        <svg x-show="toastType === 'error'" class="h-5 w-5 shrink-0 text-red-500 dark:text-red-400"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span class="text-sm font-medium" x-text="toastMessage"></span>
                        <button @click="toastVisible = false" class="mr-2 text-current opacity-60 hover:opacity-100">
                    ...
                    <button @click="performBackup()" :disabled="isBackingUp"
                        ...
                        <!-- Spinning Loader (loading state) -->
                        <svg x-show="isBackingUp" class="h-5 w-5 animate-spin text-current"
