...
<div x-data="{ searchQuery: '' }" class="flex relative flex-col gap-6 p-4 rounded-3xl bg-surface dark:bg-boxdark-2 lg:p-6 font-body" dir="rtl">
    ...
    {{-- الهيدر العلوي --}}
    <div class="flex justify-between items-center p-2 mt-6 bg-white rounded-2xl border border-gray-100 shadow-sm dark:bg-boxdark dark:border-boxdark-2 lg:p-4">
        <div class="flex flex-col">
            <h1 class="text-2xl font-black md:text-3xl font-headline text-on-surface dark:text-white">الشحنات المرسلة</h1>
    ...
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div class="relative flex-1 max-w-xl">
            ...
            <span class="absolute right-4 top-1/2 text-gray-400 transition-colors -translate-y-1/2 material-symbols-outlined dark:text-bodydark">search</span>
        </div>
    ...
    <div class="grid grid-cols-1 gap-6 md:grid-cols-2 2xl:grid-cols-3">
        @forelse($packages as $package)
            <div x-show="searchQuery === '' || '{{ $package->tracking_number }}'.includes(searchQuery) || '{{ $package->driver->name ?? '' }}'.includes(searchQuery)"
                ...
                <div class="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-primary to-primary-hover rounded-t-[24px] opacity-80"></div>

                {{-- ================= 1. الرأس (Header) ================= --}}
                <div class="flex justify-between items-start p-5">
                    <div class="flex gap-3 items-center">
                        {{-- أيقونة الإرسالية --}}
                        <div class="w-11 h-11 rounded-[14px] bg-surface dark:bg-boxdark-2 flex items-center justify-center border border-gray-100 dark:border-boxdark group-hover:scale-105 transition-transform duration-300">
                            <span class="material-symbols-outlined text-gray-500 dark:text-bodydark text-[22px]">local_shipping</span>
                        </div>
                        <div class="flex flex-col">
                    ...
                    <div class="flex gap-2 items-center">
                        ...
                        @php
                            $statusMap = [
                                'pending' => ['label' => 'قيد التجهيز', 'class' => 'bg-amber-50 text-amber-600 ring-amber-500/20 dark:bg-amber-500/10 dark:text-amber-400'],
                                'in_transit' => ['label' => 'في الطريق', 'class' => 'bg-blue-50 text-blue-600 ring-blue-500/20 dark:bg-blue-500/10 dark:text-blue-400'],
                                'delivered' => ['label' => 'مكتملة', 'class' => 'bg-emerald-50 text-emerald-600 ring-emerald-500/20 dark:bg-emerald-500/10 dark:text-emerald-400'],
                                'returned' => ['label' => 'مرتجعة', 'class' => 'bg-rose-50 text-rose-600 ring-rose-500/20 dark:bg-rose-500/10 dark:text-rose-400'],
                            ];
                            $currStatus = $statusMap[$package->status] ?? $statusMap['pending'];
                        @endphp
                        <span class="px-2.5 py-1 rounded-md text-[9px] font-black ring-1 ring-inset {{ $currStatus['class'] }}">
                            {{ $currStatus['label'] }}
                        </span>
                ...
                <div class="p-5 pt-4 space-y-5">
                    <div class="flex gap-4 justify-between items-start">
                        ...
                        <div class="flex gap-3 items-stretch w-1/2">
                            ...
                            <div class="flex flex-col flex-1 justify-between space-y-4">
                                <div>
                                    <p class="text-[9px] font-black text-gray-400 mb-0.5 tracking-wide dark:text-gray-500">فرع التجميع</p>
                                    <p class="text-xs font-bold text-on-surface truncate max-w-[100px] dark:text-gray-200">
                    ...
                    <div class="bg-boxdark rounded-[18px] p-3.5 flex justify-between items-center shadow-lg shadow-gray-900/10 dark:bg-black/30 dark:border dark:border-boxdark-2">
                        {{-- الحالة --}}
        ...
        @empty
            {{-- Empty State --}}
            <div class="flex flex-col col-span-full items-center justify-center py-20 bg-white rounded-[24px] border-2 border-dashed border-gray-200 mt-4 shadow-sm dark:bg-boxdark dark:border-boxdark-2">
                <div class="relative mb-4">
                    <div class="absolute inset-0 rounded-full blur-xl bg-primary/20"></div>
                    <div class="w-16 h-16 bg-surface dark:bg-boxdark-2 rounded-[18px] flex items-center justify-center border border-white dark:border-boxdark shadow-sm relative z-10">
                        <span class="material-symbols-outlined text-[32px] text-gray-300 dark:text-gray-600">search_off</span>
                    </div>
                </div>
                <h3 class="text-sm font-black text-on-surface dark:text-white font-headline">لا توجد إرساليات</h3>
                <p class="text-[11px] font-bold text-gray-400 dark:text-bodydark mt-1">لم نجد أي إرساليات مجمعة حتى الآن.</p>
            </div>
        @endforelse
    </div>
