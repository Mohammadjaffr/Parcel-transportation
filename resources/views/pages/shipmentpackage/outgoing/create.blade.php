...
<div class="pb-24 min-h-screen bg-surface dark:bg-boxdark-2 font-body lg:pb-12" dir="rtl"
    ...
    <form action="{{ route('shipmentpackage.outgoing.store') }}" method="POST" id="packageForm" class="flex relative flex-col"
        ...
        <div class="sticky top-0 z-50 border-b border-gray-100 shadow-sm backdrop-blur-md bg-white/90 dark:bg-boxdark/90 dark:border-boxdark-2">
            <div class="flex justify-between items-center px-4 py-4 mx-auto max-w-7xl md:px-6">
                <div class="flex gap-4 items-center">
                    ...
                    <div>
                        <h1 class="text-xl font-black leading-tight font-headline text-on-surface dark:text-white">تجهيز رحلة (إرسالية)</h1>
                        <p class="text-[11px] md:text-xs font-bold text-gray-500 dark:text-bodydark mt-0.5">تحديد السائق واختيار الطرود المتجهة معه</p>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 gap-6 items-start p-4 mx-auto w-full max-w-7xl md:p-6 lg:grid-cols-12">
            ...
            <div class="lg:col-span-4 flex flex-col gap-6 lg:sticky lg:top-[5.5rem] z-30 w-full">
                ...
                <div class="border border-gray-100 dark:border-boxdark-2 bg-white dark:bg-boxdark p-6 rounded-[2rem] shadow-sm relative overflow-hidden">
                    ...
                    <div x-data="driverSelect({{ json_encode($drivers ?? []) }}, {{ json_encode(array_values(config('countries', []))) }})" class="relative z-20">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <div class="flex relative items-center h-12 rounded-xl ring-1 ring-gray-200 transition-all dark:ring-boxdark-2 bg-surface dark:bg-boxdark-2 focus-within:ring-2 focus-within:ring-primary/40" 
                                    ...
                                    <div class="relative h-full" @click.away="openCountryDropdown = false">
                                        ...
                                        <div x-show="openCountryDropdown" x-cloak x-transition class="absolute left-0 top-full z-50 mt-2 w-64 bg-white rounded-xl border border-gray-100 shadow-xl dark:bg-boxdark-2 dark:border-boxdark">
                                            <div class="p-2 border-b border-gray-100 dark:border-boxdark">
                                                <input type="text" x-model="searchCountryQuery" placeholder="بحث..." class="px-3 py-2 w-full text-sm rounded-lg border border-gray-200 outline-none bg-surface dark:bg-boxdark dark:border-boxdark-2 dark:text-white focus:ring-1 focus:ring-primary/30" dir="rtl">
                                            </div>
                                            <div class="overflow-y-auto max-h-48 custom-scrollbar" dir="ltr">
                                                <template x-for="country in filteredCountries" :key="country.code">
                                                    <button type="button" @click="selectedCountry = country; openCountryDropdown = false; searchDriver()" class="flex gap-3 items-center px-3 py-2 w-full transition-colors hover:bg-surface dark:hover:bg-boxdark">
                                    ...
                                    <input type="tel" x-model="localPhoneNumber" @input="searchDriver" @focus="showDriverDropdown = true" @click.away="showDriverDropdown = false" placeholder="7XXXXXXXX" required autocomplete="off" :maxlength="selectedCountry?.code === 'YE' ? 9 : 15" 
                                        class="flex-1 px-3 w-full text-sm bg-transparent border-0 ring-0 outline-none font-headline text-on-surface dark:text-white"
                                        :class="selectedDriverId ? 'font-bold text-primary' : ''">
                                    
                                    <button type="button" x-show="selectedDriverId" @click="resetSelection" class="absolute right-2 z-10 p-1 text-gray-400 rounded-full transition-colors hover:text-error bg-white/80 dark:bg-boxdark/80">
                                ...
                                <div x-show="showDriverDropdown && localPhoneNumber.length > 0 && !selectedDriverId" x-cloak x-transition 
                                    ...
                                    <template x-for="driver in filteredDrivers" :key="driver.id">
                                        <button type="button" @click="selectDriver(driver)" class="flex justify-between items-center px-4 py-2.5 w-full text-right border-b border-gray-50 transition-colors hover:bg-surface dark:hover:bg-boxdark-2 dark:border-boxdark">
                            ...
                            <div>
                                ...
                                <input type="text" name="driver_name" x-model="nameInput" :readonly="selectedDriverId !== null" placeholder="اسم السائق..." required 
                                    class="px-3 w-full h-12 text-sm rounded-xl border border-gray-200 transition-all outline-none dark:bg-boxdark-2 dark:border-boxdark dark:text-white" 
                            ...
                            <div class="h-full bg-surface dark:bg-boxdark-2 p-5 rounded-[1.5rem] border border-gray-100 dark:border-boxdark flex flex-col gap-4 shadow-sm peer-checked:ring-2 peer-checked:ring-primary peer-checked:border-transparent peer-checked:bg-primary-container/20 dark:peer-checked:bg-primary/5 transition-all relative overflow-hidden hover:border-primary/30 dark:hover:border-primary/50 active:scale-[0.98]">
                                ...
                                <div class="relative z-10 pt-3 mt-auto border-t border-gray-100 dark:border-boxdark">
                                    <div class="flex flex-col gap-1.5">
                                        ...
                                        <div class="flex gap-2 items-center text-xs text-gray-500 dark:text-bodydark">
                                            <span class="material-symbols-outlined text-[16px] text-gray-400">person</span>
                                            <span class="font-medium truncate">لـ: {{ $parcel->receiverCustomer->name ?? 'عميل غير محدد' }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="md:hidden fixed bottom-6 left-4 right-4 p-4 bg-white/95 dark:bg-boxdark-2/95 backdrop-blur-md border border-gray-100 dark:border-boxdark rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] dark:shadow-black/50 flex justify-between items-center z-40">
            <div class="flex gap-3 items-center">
                ...
                <div>
                    <div class="text-[10px] font-bold text-gray-500 dark:text-bodydark uppercase tracking-widest">الطرود المحددة</div>
