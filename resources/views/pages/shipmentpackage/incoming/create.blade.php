        ...
        <div class="sticky top-0 z-[60] bg-white border-b border-slate-100 shadow-sm">
            <div class="flex justify-between items-center px-4 mx-auto max-w-7xl h-20 sm:px-6 lg:px-8">
                ...
                <div class="flex gap-4 items-center">
                    ...
                    <div>
                        <h1 class="text-lg font-black tracking-tight text-slate-800 md:text-xl font-headline">استلام إرسالية</h1>
                    ...
                    <div class="space-y-6 lg:col-span-4 lg:sticky lg:top-28">
                                    ...
                                    <div>
                                                    ...
                                                    <template x-for="country in filteredCountries" :key="country.code">
                                                        <button type="button" @click="selectedCountry = country; countryOpen = false; countrySearch = ''"
                                                            ...
                                                            <div class="flex gap-3 items-center">
                                                                <img :src="`https://flagcdn.com/w20/${country.code.toLowerCase()}.png`" class="w-5 h-auto shadow-sm">
                                        ...
                                        <div class="relative z-[50]">
                                            <div x-show="driverOpen && localPhoneNumber.trim().length > 0 && !driver_id" x-transition x-cloak @click.outside="driverOpen = false"
                                                ...
                                                <div x-show="filteredDrivers.length === 0" class="flex gap-2 justify-center items-center p-3 m-1 text-xs font-bold rounded-xl text-slate-500 bg-slate-50">
                                                    <span class="material-symbols-outlined text-[16px] text-primary">person_add</span> سائق جديد
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-6">
                            ...
                            <div class="space-y-4">
                                {{-- رقم الإرسالية --}}
                                <div>
                                    <label class="block mb-1.5 text-[11px] font-bold text-slate-500">رقم الإرسالية (التتبع) <span class="text-rose-500">*</span></label>
                                    <div class="relative group">
                                        <input type="text" name="tracking_number" value="{{ old('tracking_number') }}" required placeholder="PKG-1001"
                                ...
                                <div x-data="{
                                    ...
                                    <div>
                                        <label class="block mb-1.5 text-[11px] font-bold text-slate-500">المكتب الخارجي <span class="text-rose-500">*</span></label>
                                    ...
                                    <div>
                                        <label class="block mb-1.5 text-[11px] font-bold text-slate-500">الفرع <span class="text-rose-500">*</span></label>
                                        <div class="relative">
                                            <select name="sender_office_branch_id" x-model="selectedBranch" required :disabled="!selectedOffice"
                    ...
                    <div class="lg:col-span-8">
                        <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm">
                            ...
                            <div class="flex justify-between items-center px-6 py-5 border-b border-slate-50">
                                <div class="flex gap-3 items-center">
                                    <div class="flex justify-center items-center w-10 h-10 text-emerald-500 bg-emerald-50 rounded-xl shrink-0">
                            ...
                            <div class="p-6 space-y-4">
                                    ...
                                    <div :class="errorIndices.includes(index) ? 'border-rose-300 shadow-sm' : (activeItem === index ? 'border-primary/40 shadow-[0_8px_30px_-10px_rgba(var(--color-primary-rgb),0.3)]' : 'border-slate-100 hover:border-slate-200')"
                                            ...
                                            <div class="flex gap-4 items-center">
                                                ...
                                                <div class="flex flex-col">
                                                    <h4 class="text-sm font-black transition-colors"
                                                        ...
                                                        x-text="item.bond_number ? 'طرد رقم: ' + item.bond_number : 'طرد جديد (بدون رقم)'"></h4>
                                                    <p class="text-[11px] font-bold mt-0.5"
                                                        :class="activeItem === index ? 'text-slate-500' : 'text-slate-400'"
                                                        x-text="item.receiver_name ? 'إلى المستلم: ' + item.receiver_name : 'يرجى إكمال البيانات'"></p>
                                                </div>
                                            </div>

                                            <div class="flex gap-3 items-center">
                                                <button type="button" @click.stop="removeItem(index)" x-show="items.length > 1"
                                        ...
                                        <div x-show="activeItem === index" x-collapse>
                                            <div class="p-6 space-y-6">
                                                ...
                                                {{-- رقم السند --}}
                                                <div>
                                                    <label class="block mb-1.5 text-[11px] font-bold text-slate-500">رقم السند / البوليصة <span class="text-rose-500">*</span></label>
                                                    <input type="text" :name="`items[${index}][bond_number]`" x-model="item.bond_number" placeholder="أدخل رقم السند" required
                                                ...
                                                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                                                    ...
                                                    <div class="p-4 space-y-4 rounded-2xl border bg-slate-50/50 border-slate-100"
                                                        ...
                                                        <h5 class="pb-2 text-xs font-black border-b text-slate-700 border-slate-200">بيانات المرسل</h5>
                                                        
                                                        <div class="relative z-[40]">
                                                            <label class="block mb-1.5 text-[10px] font-bold text-slate-500">رقم الهاتف</label>
                                                            <div class="flex overflow-visible items-center w-full h-11 bg-white rounded-lg border border-slate-200 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20">
                                                                <input type="tel" x-model="sdLocal" @input="sdOpen = true" @focus="sdOpen = true" @click.outside="sdOpen = false"
                                                                    placeholder="7XXXXXXXX" dir="ltr" autocomplete="off" :maxlength="sdSelected?.code === 'YE' ? 9 : 15"
                                                                ...
                                                                <button type="button" @click="sdOpen = !sdOpen" class="flex gap-1.5 items-center px-2.5 h-full rounded-l-lg border-r bg-slate-50 border-slate-200 shrink-0">
                                                                    <template x-if="sdSelected">
                                                                        <div class="flex gap-1.5 items-center">
                                                                            <img :src="`https://flagcdn.com/w20/${sdSelected.code.toLowerCase()}.png`" class="w-4 h-auto shadow-sm">
                                                                            <span class="text-[11px] font-bold text-slate-600" dir="ltr" x-text="sdSelected.dial_code"></span>
                                                                        </div>
                                                                    </template>
                                                                </button>
                                                            </div>
                                                            <div x-show="sdOpen && sdLocal.trim().length > 0" x-transition x-cloak class="overflow-y-auto absolute right-0 left-0 p-1 mt-1 max-h-48 bg-white rounded-xl border shadow-xl border-slate-100 custom-scrollbar">
                                                                <template x-for="customer in sdFilteredCustomers" :key="customer.id">
                                                                    <button type="button" @click="selectCustomer(customer)" class="flex justify-between items-center px-3 py-2.5 w-full text-right rounded-lg border-b border-slate-50 hover:bg-primary/5">
                                                                        <span class="text-xs font-black text-slate-800" x-text="customer.name"></span>
                                                        ...
                                                        <div>
                                                            ...
                                                            <input type="text" :name="`items[${index}][sender_name]`" x-model="item.sender_name" placeholder="الاسم"
                                                    ...
                                                    <div class="p-4 space-y-4 rounded-2xl border bg-slate-50/50 border-slate-100"
                                                        ...
                                                        <h5 class="pb-2 text-xs font-black border-b text-slate-700 border-slate-200">بيانات المستلم</h5>
                                                        
                                                        <div class="relative z-[30]">
                                                            <label class="block mb-1.5 text-[10px] font-bold text-slate-500">رقم الهاتف <span class="text-rose-500">*</span></label>
                                                            <div class="flex overflow-visible items-center w-full h-11 bg-white rounded-lg border border-slate-200 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20">
                                                                <input type="tel" x-model="rcLocal" @input="rcOpen = true" @focus="rcOpen = true" @click.outside="rcOpen = false"
                                                                    placeholder="7XXXXXXXX" dir="ltr" autocomplete="off" required :maxlength="rcSelected?.code === 'YE' ? 9 : 15"
                                                                    class="flex-grow px-3 w-full min-w-0 h-full text-sm font-bold tracking-wider text-left bg-transparent border-none outline-none focus:ring-0 text-slate-700">
                                                                
                                                                <button type="button" @click="rcOpen = !rcOpen" class="flex gap-1.5 items-center px-2.5 h-full rounded-l-lg border-r bg-slate-50 border-slate-200 shrink-0">
                                                                    <template x-if="rcSelected">
                                                                        <div class="flex gap-1.5 items-center">
                                                                            <img :src="`https://flagcdn.com/w20/${rcSelected.code.toLowerCase()}.png`" class="w-4 h-auto shadow-sm">
                                                                            <span class="text-[11px] font-bold text-slate-600" dir="ltr" x-text="rcSelected.dial_code"></span>
                                                                        </div>
                                                                    </template>
                                                                </button>
                                                            </div>
                                                            <div x-show="rcOpen && rcLocal.trim().length > 0" x-transition x-cloak class="overflow-y-auto absolute right-0 left-0 p-1 mt-1 max-h-48 bg-white rounded-xl border shadow-xl border-slate-100 custom-scrollbar">
                                                ...
                                                <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                                                    ...
                                                    <div class="md:col-span-1">
                                                        <label class="block mb-1.5 text-[11px] font-bold text-slate-500">التحصيل <span class="text-rose-500">*</span></label>
                                                        <div class="flex p-1 rounded-xl border bg-slate-100 border-slate-200/50">
                                                            <label class="flex-1 cursor-pointer">
                                                                ...
                                                                <div class="flex justify-center items-center w-full h-9 text-xs font-black rounded-lg transition-all text-slate-400 peer-checked:bg-white peer-checked:text-primary peer-checked:shadow-sm">
                                                                    عند الاستلام
                                                                </div>
                                                            </label>
                                                            <label class="flex-1 cursor-pointer">
                                                                ...
                                                                <div class="flex justify-center items-center w-full h-9 text-xs font-black rounded-lg transition-all text-slate-400 peer-checked:bg-white peer-checked:text-emerald-500 peer-checked:shadow-sm">
                                                                    مدفوع
                                                                </div>
                                                            </label>
                                                        </div>
                                                    ...
                                                    {{-- المبلغ --}}
                                                    <div class="md:col-span-1">
                                                        <label class="block mb-1.5 text-[11px] font-bold text-slate-500">المبلغ (ر.ي) <span class="text-rose-500">*</span></label>
                                                    ...
                                                    {{-- نوع الشحنة --}}
                                                    <div class="relative z-[20] md:col-span-1" x-data="{ isOpen: false, options: ['كرتون', 'كيس', 'مغلف', 'طرد'] }">
                                                        <label class="block mb-1.5 text-[11px] font-bold text-slate-500">نوع الشحنة <span class="text-rose-500">*</span></label>
                                                        <input type="text" :name="`items[${index}][package_type]`" x-model="item.package_type" 
                                                            @focus="isOpen = true" @click.away="isOpen = false" placeholder="اختر النوع..." required
                                                        ...
                                                        <button type="button" @click="isOpen = !isOpen" class="flex absolute bottom-1 left-1 justify-center items-center w-9 h-9 rounded-lg text-slate-400 hover:text-primary">
                                                            <span class="material-symbols-outlined text-[20px]" :class="isOpen ? 'rotate-180' : ''">expand_more</span>
                                                        </button>
                                                        <div x-show="isOpen" x-cloak x-transition class="overflow-hidden absolute right-0 left-0 py-1 mt-1 bg-white rounded-xl border shadow-xl border-slate-100">
                                                            <template x-for="option in options" :key="option">
                                                                <button type="button" @click="item.package_type = option; isOpen = false" class="flex justify-between items-center px-4 py-2.5 w-full text-xs font-bold text-right text-slate-700 hover:bg-slate-50">
                                                ...
                                                {{-- ملاحظات الطرد --}}
                                                <div>
                                                    <label class="block mb-1.5 text-[11px] font-bold text-slate-500">ملاحظات إضافية على الطرد</label>
                                                    <input type="text" :name="`items[${index}][item_notes]`" x-model="item.item_notes" placeholder="محتوى قابل للكسر، إلخ..."
                                ...
                                {{-- حالة عدم وجود طرود --}}
                                <div x-show="items.length === 0" class="flex flex-col justify-center items-center py-12 rounded-[1.5rem] border-2 border-dashed bg-slate-50/50 border-slate-200">
                                    ...
                                    <button type="button" @click="addItem()" class="px-5 py-2.5 mt-4 text-xs font-black text-emerald-600 bg-emerald-100 rounded-xl transition-colors hover:bg-emerald-200">
                                        إضافة الطرد الأول
                                    </button>
                                </div>
                            </div>
