...
@section('content')
    <div class="flex flex-col gap-5 px-4 pt-4 pb-24 min-h-screen bg-slate-50/50">
        ...
        <form action="{{ route('shipmentpackage.incoming.store') }}" method="POST" class="space-y-5" dir="rtl"
            ...
            <div
                class="bg-white rounded-[2rem] border border-slate-100 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)] overflow-hidden">
                ...
                <div class="p-5 space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative group">
                            ...
                            <div
                                class="flex absolute inset-y-0 right-0 items-center pr-3.5 transition-colors pointer-events-none text-slate-400 group-focus-within:text-primary">
                                <span class="material-symbols-outlined text-[20px]">tag</span>
                            </div>
                        </div>
                        @error('tracking_number')
                            <p class="mt-1 text-[10px] font-bold text-rose-500">{{ $message }}</p>
                        @enderror
                    </div>
                            ...
                            <div class="relative">
                                <select name="sender_office_branch_id" x-model="selectedBranch" required
                                    ...
                                    :class="!selectedOffice ?
                                        ...
                                        'bg-slate-50/50 focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 text-slate-700 border-slate-200'"
                                    ...
                                    <template x-for="branch in currentBranches" :key="branch.id">
                                        <option :value="branch.id"
                                            x-text="branch.name + (branch.city ? ' (' + branch.city + ')' : '')"></option>
                                    </template>
                                </select>
                    ...
                    <div class="p-4 space-y-4 rounded-2xl border bg-slate-50 border-slate-100">
                        <div class="flex gap-2 items-center">
                            ...
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-wider">بيانات
                                السائق</span>
                        </div>
                        ...
                        <div>
                            <label class="block mb-1.5 text-[11px] font-bold text-slate-500">رقم هاتف السائق <span
                            ...
                            <div class="flex overflow-visible relative items-center w-full h-12 bg-white rounded-xl border transition-all border-slate-200 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20"
                                ...
                                <button type="button" @click="countryOpen = !countryOpen"
                                    ...
                                    <img :src="`https://flagcdn.com/w20/${selectedCountry.code.toLowerCase()}.png`"
                                        class="w-4 sm:w-5 h-auto rounded-[2px] shadow-sm">
                                    <span class="text-[11px] sm:text-xs font-bold text-slate-600" dir="ltr"
                                ...
                                <div x-show="driverOpen && localPhoneNumber.trim().length > 0 && !driver_id" x-transition
                                    x-cloak @click.outside="driverOpen = false"
                                    ...
                                    <div x-show="filteredDrivers.length === 0"
                                        class="flex gap-2 justify-center items-center px-4 py-4 m-1 text-xs font-bold rounded-xl text-slate-500 bg-slate-50">
                        ...
                        <div>
                            ...
                            <input type="text" name="driver_name" x-model="driver_name"
                                ...
                                :class="driver_id ? 'bg-slate-100/50 text-slate-400 border-slate-200 cursor-not-allowed' :
                                    'bg-white border-slate-200 focus:border-primary focus:ring-2 focus:ring-primary/20 text-slate-700'"
            ...
            <div
                class="bg-white rounded-[2rem] border border-slate-100 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)] overflow-visible relative">

                {{-- Header --}}
                <div
                    class="flex justify-between items-center px-5 py-4 border-b border-slate-50 bg-slate-50/50 rounded-t-[2rem]">
                    <div class="flex gap-3 items-center">
                        <div
                            class="flex justify-center items-center w-10 h-10 rounded-xl bg-secondary/10 text-secondary shrink-0">
                        ...
                        <div :class="errorIndices.includes(index) ? 'border-rose-300 shadow-sm' : (activeItem === index ?
                            ...
                            <div @click="activeItem = activeItem === index ? null : index"
                                ...
                                <div class="flex gap-3 items-center">
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <p class="text-[10px] font-bold mt-0.5"
                                            ...
                                            x-text="item.receiver_name ? 'للمستلم: ' + item.receiver_name : 'يرجى إكمال البيانات...'">
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="flex gap-2 items-center">
                                    ...
                                    <div class="flex justify-center items-center w-8 h-8 rounded-full transition-colors"
                                        :class="activeItem === index ? 'bg-primary/10 text-primary' :
                            ...
                            <div x-show="activeItem === index" x-collapse>
                                <div class="p-5 space-y-5">
                                    ...
                                    {{-- رقم السند --}}
                                    <div>
                                        <label class="block mb-1.5 text-[10px] font-bold text-slate-500">رقم البوليصة
                                            (السند) <span class="text-rose-500">*</span></label>
                                    ...
                                    {{-- ================= بيانات المرسل (ذكي) ================= --}}
                                    <div class="p-3 space-y-3 rounded-xl border bg-slate-50/80 border-slate-100"
                                        x-data="{
                                            sdOpen: false,
                                            sdSearch: '',
                                            sdLocal: '',
                                        ...
                                        }" x-init="sdSelected = sdCountries.find(c => c.code === 'YE') || sdCountries[0];
                                        ...
                                        {{-- هاتف المرسل --}}
                                        <div class="relative">
                                            <label class="block mb-1.5 text-[10px] font-bold text-slate-500">رقم هاتف
                                                المرسل</label>
                                            ...
                                            <div
                                                ...
                                                <button type="button" @click="sdOpen = !sdOpen"
                                                    class="flex gap-1.5 items-center px-2.5 h-full rounded-l-lg border-r transition-colors bg-slate-50 border-slate-200 shrink-0 hover:bg-slate-100">
                                                    <template x-if="sdSelected">
                                                        <div class="flex gap-1.5 items-center">
                                                            <img :src="`https://flagcdn.com/w20/${sdSelected.code.toLowerCase()}.png`"
                                                                class="w-4 h-auto rounded-[2px] shadow-sm">
                                                            <span class="text-[10px] font-bold text-slate-600"
                                                                dir="ltr" x-text="sdSelected.dial_code"></span>
                                                        </div>
                                                    </template>
                                                </button>
                                            </div>
                                            ...
                                            <div x-show="sdOpen && sdLocal.trim().length > 0" x-transition x-cloak
                                                ...
                                                <div x-show="sdFilteredCustomers.length === 0"
                                                    class="flex gap-1.5 items-center justify-center px-3 py-3 text-[10px] font-bold text-slate-500 bg-slate-50 rounded-lg m-1">
                                                    <span
                                                        class="material-symbols-outlined text-[14px] text-primary">person_add</span>
                                        ...
                                        {{-- اسم المرسل --}}
                                        <div>
                                            <label class="block mb-1.5 text-[10px] font-bold text-slate-500">اسم
                                                المرسل</label>
                                            <input type="text" :name="`items[${index}][sender_name]`"
                                                x-model="item.sender_name" placeholder="اختياري"
                                    ...
                                    <div class="p-3 space-y-3 rounded-xl border bg-slate-50/80 border-slate-100"
                                        ...
                                        }" x-init="rcSelected = rcCountries.find(c => c.code === 'YE') || rcCountries[0];
                                                    ...
                                                    <template x-if="rcSelected">
                                                        <div class="flex gap-1.5 items-center">
                                                            <img :src="`https://flagcdn.com/w20/${rcSelected.code.toLowerCase()}.png`"
                                                                class="w-4 h-auto rounded-[2px] shadow-sm">
                                                            <span class="text-[10px] font-bold text-slate-600"
                                                                dir="ltr" x-text="rcSelected.dial_code"></span>
                                                        </div>
                                                    </template>
                                                </button>
                                            </div>
                                        ...
                                        {{-- اسم المستلم --}}
                                        <div>
                                            <label class="block mb-1.5 text-[10px] font-bold text-slate-500">اسم المستلم
                                    ...
                                    <div class="grid grid-cols-2 gap-3">
                                        <div>
                                            <label class="block mb-1.5 text-[10px] font-bold text-slate-500">حالة الدفع
                                                <span class="text-rose-500">*</span></label>
                                            <div class="flex p-1 rounded-xl border bg-slate-100 border-slate-200/50">
                                                <label class="flex-1 cursor-pointer">
                                                    ...
                                                    <div
                                                        class="flex justify-center items-center w-full h-9 text-[10px] font-black text-slate-400 rounded-lg transition-all peer-checked:bg-white peer-checked:text-primary peer-checked:shadow-sm">
                                                        عند الاستلام
                                                    </div>
                                                </label>
                                                <label class="flex-1 cursor-pointer">
                                                    <input type="radio" value="paid"
                                                        ...
                                                        class="sr-only peer">
                                                    <div
                                                        class="flex justify-center items-center w-full h-9 text-[10px] font-black text-slate-400 rounded-lg transition-all peer-checked:bg-white peer-checked:text-emerald-500 peer-checked:shadow-sm">
                                                        مدفوع
                                                    </div>
                                                </label>
                                            </div>
                                        ...
                                        <div>
                                            <label class="block mb-1.5 text-[10px] font-bold text-slate-500">المبلغ (ر.ي)
                                                <span class="text-rose-500">*</span></label>
                                            <input type="number" :name="`items[${index}][amount]`" x-model="item.amount"
                                                ...
                                                :class="item.payment_status === 'paid' ?
                                                    'bg-slate-100 text-slate-400 border-slate-200 cursor-not-allowed' :
                                                    'bg-slate-50/50 border-slate-200 focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 text-slate-700'"
    ...
    <div x-data="{ isOpen: false, options: ['كرتون', 'كيس', 'مغلف', 'طرد'] }" class="relative">
        ...
        <div x-show="isOpen" x-cloak x-transition.opacity.scale.origin.top.left
            ...
            <template x-for="option in options" :key="option">
                <button type="button"
                    ...
                    <div x-show="items.length === 0"
                        class="flex flex-col justify-center items-center py-10 rounded-2xl border border-dashed bg-slate-50 border-slate-200">
            ...
            {{-- ======================== Footer (Submit) ======================== --}}
            <div
                class="mt-8 mb-4 p-5 bg-white rounded-[2.5rem] border border-slate-50 shadow-[0_15px_50px_-15px_rgba(0,0,0,0.05)] flex flex-col items-center justify-center">
                <button type="submit"
