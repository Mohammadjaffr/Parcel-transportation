...
@section('title', 'تعديل الإرسالية الواردة - ' . $package->id)
...
@section('content')
    ...
    <div class="pb-28 min-h-screen bg-slate-50/50 font-body" dir="rtl" x-data="{
        ...
        localPhoneNumber: '', // سيتم تهيئته في x-init
    ...
    if (this.items.length > 1) {
        ...
        <div class="sticky top-0 z-[60] bg-white border-b border-slate-100 shadow-sm">
            <div class="flex justify-between items-center px-4 mx-auto max-w-7xl h-20 sm:px-6 lg:px-8">
                ...
                <div class="flex gap-4 items-center">
                    ...
                    <div>
                        <h1 class="text-lg font-black tracking-tight text-slate-800 md:text-xl font-headline">تعديل الإرسالية
                        </h1>
                        <p class="text-[11px] font-bold text-slate-500 mt-0.5">تحديث الرحلة #{{ $package->id }} وإدارة طرودها</p>
                    </div>
                </div>
                ...
                <div class="flex gap-4 items-center">
                    ...
                    <button type="button" @click="document.getElementById('editShipmentForm').requestSubmit()"
        ...
        <div class="px-4 pt-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
            <form id="editShipmentForm" action="{{ route('shipmentpackage.incoming.update', $package->id) }}" method="POST"
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <textarea name="notes" rows="2" placeholder="اختياري..."
                                        class="p-4 w-full text-sm font-bold rounded-xl border transition-all outline-none resize-none bg-slate-50 border-slate-200 focus:bg-white focus:border-primary text-slate-700">{{ old('notes', $package->notes) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="p-6 space-y-4">
                                <template x-for="(item, index) in items" :key="index">
                                    <div :class="errorIndices.includes(index) ? 'border-rose-300 shadow-sm' : (activeItem === index ? 'border-primary/40 shadow-[0_8px_30px_-10px_rgba(var(--color-primary-rgb),0.3)]' : 'border-slate-100 hover:border-slate-200')"
                                        ...
                                        {{-- 💡 الحقل المخفي الخاص بـ id الطرد لضمان التحديث --}}
                                       ...
                                       <div class="flex gap-3 items-center">
    {{-- تمت إضافة x-show وتعديل @click.stop ليستدعي الموديل --}}
   <button type="button" x-show="index > 0" @click.stop="removeItem(index)"
            ...
            :class="activeItem === index ? 'rotate-180' : ''">expand_more</span>
                                        ...
                                        <div x-show="activeItem === index" x-collapse>
                                            <div class="p-6 space-y-6">
                                                ...
                                                {{-- رقم السند --}}
                                                <div>
                                                    <label class="block mb-1.5 text-[11px] font-bold text-slate-500">رقم السند / الطرد <span class="text-rose-500">*</span></label>
                                                ...
                                                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                                                    ...
                                                    <div class="p-4 space-y-4 rounded-2xl border bg-slate-50/50 border-slate-100"
                                                        ...
                                                        <h5 class="pb-2 text-xs font-black border-b text-slate-700 border-slate-200">بيانات المرسل (اختياري)</h5>

                                                        <div class="relative z-[40]">
                                                            <label class="block mb-1.5 text-[10px] font-bold text-slate-500">رقم الهاتف (اختياري)</label>
                                                        ...
                                                        <div>
                                                            <label class="block mb-1.5 text-[10px] font-bold text-slate-500">اسم المرسل (اختياري)</label>
                                                            <input type="hidden" :name="`items[${index}][sender_phone]`" x-model="item.sender_phone">
                                                            <input type="text" :name="`items[${index}][sender_name]`" x-model="item.sender_name" placeholder="الاسم (اختياري)"
                                                    ...
                                                    <div class="p-4 space-y-4 rounded-2xl border bg-slate-50/50 border-slate-100"
                                                        ...
                                                        <div>
                                                            ...
                                                            <input type="hidden" :name="`items[${index}][receiver_phone]`" x-model="item.receiver_phone">
                                                ...
                                                <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                                                    ...
                                                    {{-- نوع الشحنة --}}
                                                    <div class="relative z-[20] md:col-span-1" x-data="{ isOpen: false, options: ['كرتون', 'كيس', 'مغلف', 'أخرى'] }">
                                                        ...
                                                        <input type="text" :name="`items[${index}][package_type]`"
                                                            ...
                                                            @click.away="isOpen = false" placeholder="اختر النوع..." required
                                ...
                                {{-- حالة عدم وجود طرود مستحيلة لأننا منعنا حذفها بالكامل، ولكن كإجراء احتياطي --}}
