...
@section('content')
    ...
    <div x-data="driverFilter()" @open-create-modal.window="showCreateModal = true" class="space-y-6 font-outfit" dir="rtl">
        ...
        <div class="grid grid-cols-1 gap-4 xl:grid-cols-3 md:gap-6">
            ...
            <div @click="statusFilter = 'all'"
                ...
                <div class="mt-3">
                    ...
                    <h4 class="text-xl font-black dark:text-white" 
                        x-text="search === '' && statusFilter === 'all' ? '{{ $drivers->count() }} من {{ $drivers->total() ?? 0 }}' : filteredDrivers.length"></h4>
                </div>
            </div>

            {{-- متاحين للعمل --}}
            <div @click="statusFilter = 'available'"
                :class="statusFilter === 'available' ? 'border-success-500 ring-2 ring-success-500/20' : 'border-gray-100 hover:border-success-300 dark:border-gray-800'"
                class="relative flex flex-col items-start justify-between p-5 transition-all bg-white border cursor-pointer rounded-2xl dark:bg-white/[0.03] hover:shadow-md shadow-theme-sm">
            ...
            <div @click="statusFilter = 'on_trip'"
                :class="statusFilter === 'on_trip' ? 'border-warning-500 ring-2 ring-warning-500/20' : 'border-gray-100 hover:border-warning-300 dark:border-gray-800'"
                class="relative flex flex-col items-start justify-between p-5 transition-all bg-white border cursor-pointer rounded-2xl dark:bg-white/[0.03] hover:shadow-md shadow-theme-sm">
        ...
        <div class="bg-white dark:bg-boxdark rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-theme-sm overflow-hidden transition-colors">
            ...
            <div class="p-4 w-full bg-white rounded-2xl border-b border-gray-100 dark:bg-transparent dark:border-gray-800">
                <div class="relative rounded-2xl border border-gray-200 transition-all group focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 dark:border-gray-700">
                    <input type="text" x-model.debounce.300ms="search"
                        placeholder="ابحث باسم السائق أو رقم الهاتف (في هذه الصفحة)..."
            ...
            <div class="flex flex-col gap-4 p-4 lg:hidden">
                <template x-for="(driver, index) in filteredDrivers" :key="driver.id">
                    <div class="flex flex-col gap-3 p-4 rounded-xl border border-gray-100 transition-all bg-gray-50/50 dark:bg-gray-800/50 dark:border-gray-700 hover:border-primary/30 hover:shadow-sm">
                        ...
                        <div class="flex justify-between items-center pt-3 border-t border-gray-100 dark:border-gray-800">
                            <span :class="driver.status === 'available' ? 'bg-success-50 text-success-600 dark:bg-success-500/10 dark:text-success-400' : 'bg-warning-50 text-warning-600 dark:bg-warning-500/10 dark:text-warning-400'"
                                class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-medium">
                                <span :class="driver.status === 'available' ? 'bg-success-500' : 'bg-warning-500'" class="w-1.5 h-1.5 rounded-full"></span>
                                <span x-text="driver.status === 'available' ? 'متاح للعمل' : 'في رحلة التوصيل'"></span>
                            </span>
                            
                            <a :href="`/drivers/${driver.id}/shipments`"
                                class="text-xs font-semibold transition-colors text-primary hover:text-primary-hover">
                                عرض الشحنات &larr;
                            </a>
                        </div>
                    </div>
                ...
                {{-- حالة عدم وجود نتائج --}}
                <div x-show="filteredDrivers.length === 0" x-cloak
                    ...
                    <div class="flex flex-col justify-center items-center">
                        ...
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">لا يوجد سائق يطابق معايير البحث أو التصفية.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="hidden overflow-x-auto px-4 pb-4 mt-4 lg:block">
                <table class="w-full text-right border-separate border-spacing-y-3">
                    ...
                    <tbody class="divide-y-0">
                        <template x-for="(driver, index) in filteredDrivers" :key="driver.id">
                            <tr class="bg-white rounded-2xl border border-transparent shadow-sm transition-all dark:bg-gray-900/50 hover:shadow-md hover:border-primary/30 dark:hover:border-primary/30">
                                ...
                                <td class="px-6 py-5 border-gray-100 border-y dark:border-gray-800">
                                    <div class="flex gap-3 items-center">
                                        <div class="flex justify-center items-center w-10 h-10 text-sm font-bold text-white rounded-full shadow-sm bg-primary" x-text="driver.name ? driver.name.charAt(0) : '?'"></div>
                                        <span class="text-sm font-black text-gray-900 dark:text-white" x-text="driver.name"></span>
                                    </div>
                                </td>
                        ...
                        <tr x-show="filteredDrivers.length === 0" x-cloak>
            ...
            return {
                ...
                get filteredDrivers() {
                    ...
                    if (this.statusFilter !== 'all') {
                        // تأكد أن الحقل في الداتابيز اسمه status ويحمل القيم available أو on_trip
