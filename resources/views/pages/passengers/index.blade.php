    ...
    <div x-data="passengerRegistry()" class="pb-24 space-y-6 min-h-screen font-body lg:pb-12" dir="rtl">
        ...
        <div class="grid grid-cols-1 gap-2 mx-auto max-w-7xl xl:grid-cols-3 md:gap-6">
            <div class="flex relative flex-col justify-between items-start p-5 bg-white rounded-2xl border ring-2 shadow-sm transition-all border-primary ring-primary/20 dark:bg-boxdark">
                ...
                <div class="mt-4">
                    <span class="text-xs font-bold tracking-widest text-gray-500 uppercase dark:text-bodydark">إجمالي الركاب</span>
            ...
            <div class="flex relative flex-col justify-between items-start p-5 bg-white rounded-2xl border border-r-4 border-gray-100 shadow-sm transition-all dark:bg-boxdark border-r-emerald-500 dark:border-r-emerald-500 dark:border-boxdark-2">
                ...
                <div class="mt-4">
                    ...
                    <h4 class="mt-1 text-2xl font-black text-on-surface dark:text-white">{{ number_format($passengers->getCollection()->sum('count'), 0) }}</h4>
                </div>
            </div>
            ...
            <div class="flex relative flex-col justify-between items-start p-5 bg-white rounded-2xl border border-r-4 border-gray-100 shadow-sm transition-all dark:bg-boxdark border-r-amber-500 dark:border-r-amber-500 dark:border-boxdark-2">
                ...
                <div class="mt-4">
                    ...
                    <h4 class="mt-1 text-2xl font-black text-on-surface dark:text-white">{{ number_format($passengers->getCollection()->sum('total_commission'), 0) }}</h4>
                </div>
            </div>
        </div>
                ...
                <div class="flex flex-col gap-4 justify-between items-stretch md:flex-row md:items-center">
                    <div class="flex flex-col gap-3 w-full md:flex-row md:items-center">
                        <div class="relative w-full rounded-2xl border border-gray-200 transition-all md:w-[420px] dark:border-boxdark-2 group focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 bg-surface dark:bg-boxdark-2">
                            ...
                            <button type="button" x-show="searchQuery.length > 0" @click="searchQuery = ''; updateVisibility()" x-cloak
                        ...
                        <div class="relative w-full rounded-2xl border border-gray-200 transition-all md:w-56 dark:border-boxdark-2 bg-surface dark:bg-boxdark-2 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20">
                            ...
                            <div class="flex absolute inset-y-0 right-0 items-center pr-4 text-gray-400 pointer-events-none"><span class="material-symbols-outlined text-[20px]">tune</span></div>
                            <div class="flex absolute inset-y-0 left-0 items-center pl-3 text-gray-400 pointer-events-none"><span class="material-symbols-outlined text-[20px]">expand_more</span></div>
                        </div>
                    </div>
                    ...
                    <div class="flex gap-2 items-center text-xs font-black text-gray-500 dark:text-bodydark">
                        ...
                        <span>النتائج المعروضة: <span class="text-primary" x-text="visibleCount"></span> من <span>{{ $passengers->count() }}</span></span>
                    </div>
                </div>
            </div>
                            ...
                            <tr class="transition-colors hover:bg-gray-50/80 dark:hover:bg-boxdark-2/50 group passenger-row"
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col gap-1">
                                        <span class="text-sm font-black text-gray-800 dark:text-white">{{ optional($passenger->date)->format('Y-m-d') }}</span>
                                        <span class="text-[11px] font-bold text-gray-500 dark:text-bodydark">{{ $dayName }}</span>
                                    </div>
                                </td>

                                <td class="px-6 py-4">
                                    <span class="text-sm font-black text-primary font-mono dir-ltr">{{ $passenger->passenger_number }}</span>
                                </td>

                                <td class="px-6 py-4">
                                    <div class="flex gap-3 items-center">
                                        <div class="flex justify-center items-center w-10 h-10 text-lg font-black text-white rounded-lg shadow-inner bg-slate-800 dark:bg-slate-700">
                                            {{ mb_substr($passenger->customer->name ?? 'ع', 0, 1, 'UTF-8') }}
                                        </div>
                                        <div class="flex flex-col gap-1 min-w-0">
                                            <span class="text-sm font-bold text-gray-700 dark:text-gray-300">{{ $passenger->customer->name ?? 'غير محدد' }}</span>
                                            <x-phone-number :value="$passenger->customer->phone ?? ''" class="text-[11px] font-bold text-gray-500 dark:text-bodydark" />
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex flex-col gap-1 items-center">
                                        <span class="text-xs font-black text-primary">{{ $passenger->driver->name ?? 'غير محدد' }}</span>
                                        <x-phone-number :value="$passenger->driver->phone ?? ''" class="text-[10px] font-bold text-gray-500 dark:text-bodydark" />
                                    </div>
                                </td>

                                <td class="px-6 py-4 text-center">
                                    <span class="px-3 py-1.5 text-xs font-bold text-gray-600 bg-white rounded-lg border border-gray-100 shadow-sm dark:bg-boxdark dark:text-gray-300 dark:border-boxdark-2">{{ $passenger->location ?: 'غير محدد' }}</span>
                                </td>

                                <td class="px-6 py-4 text-center">
                                    <span class="inline-flex gap-1.5 items-center justify-center px-3 py-1.5 rounded-xl text-[11px] font-black {{ $statusClass }}">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex flex-col gap-1 items-center">
                                        <span class="px-3 py-1.5 rounded-lg text-[10px] font-black bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400">العدد: {{ $passenger->count ?? 0 }}</span>
                                        <span class="text-xs font-black text-amber-600 dark:text-amber-400">{{ number_format($passenger->total_commission ?? 0, 0) }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="relative px-6 py-4 text-center">
                                    <div x-data="{ open: false }" class="inline-block relative text-right" @click.away="open = false">
                                        <button @click="open = !open" type="button" title="خيارات" class="inline-flex justify-center items-center w-9 h-9 text-gray-400 bg-white rounded-lg border border-gray-100 shadow-sm transition-all hover:bg-gray-100 hover:text-gray-600 hover:border-gray-200 dark:bg-boxdark dark:border-boxdark-2 dark:hover:bg-boxdark-2 dark:hover:text-gray-300 active:scale-95">
                                            <span class="material-symbols-outlined text-[20px]">more_vert</span>
                                        </button>

                                        <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute left-0 top-full mt-2 z-[999] w-52 bg-white/95 backdrop-blur-md rounded-xl border border-gray-100 shadow-xl dark:bg-boxdark/95 dark:border-boxdark-2 focus:outline-none origin-top-left overflow-hidden" style="display: none;">
                                            <div class="py-1" role="menu">
                                                <a href="{{ route('passengers.show', $passenger->id) }}" class="flex gap-3 items-center px-4 py-2.5 w-full text-xs font-bold text-gray-700 transition-colors dark:text-gray-200 hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-boxdark-2 dark:hover:text-blue-400">
                                                    <span class="material-symbols-outlined text-[18px]">visibility</span> عرض التفاصيل
                                                </a>
...
@if($statusKey === 'pending')
    <button type="button" @click="open = false; openStatusModal({
        ...
        <span class="material-symbols-outlined text-[18px]">fact_check</span> تعيين الحالة
    </button>
    <div class="mx-3 my-1 h-px bg-gray-100 dark:bg-boxdark"></div>
@endif
                                                ...
                                                <button type="button" @click="open = false; openDeleteModal({{ $passenger->id }}, {{ json_encode($passenger->passenger_number) }})"
                                                    ...
                                                    <span class="material-symbols-outlined text-[18px]">delete</span> حذف الراكب
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <tr>
                                <td colspan="8" class="py-24 text-center">
                                    <div class="flex flex-col gap-4 justify-center items-center">
                                        ...
                                        <div>
                                            <h3 class="mb-1 text-base font-bold text-gray-800 dark:text-white">لا توجد بيانات للركاب</h3>
                                            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">لم نعثر على أي ركاب مسجلين في النظام حالياً.</p>
                                        </div>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <td colspan="8" class="py-24 text-center">
                                <div class="flex flex-col gap-4 justify-center items-center">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">لم نعثر على ركاب يطابقون بحثك أو الحالة المحددة.</p>
                                    </div>
                                </div>
                            </td>
                        </tr>
        ...
        <div x-show="showCreateModal" x-cloak
            ...
            <div class="relative w-full max-w-4xl bg-white dark:bg-boxdark rounded-[2rem] shadow-2xl border border-gray-100 dark:border-boxdark-2 p-6 md:p-8 pointer-events-auto max-h-[90vh] overflow-y-auto custom-scrollbar">
                ...
                <div class="flex justify-between items-center pb-4 mb-6 border-b border-gray-50 dark:border-boxdark-2">
                    ...
                    <button type="button" @click="closeModals()" class="flex justify-center items-center w-10 h-10 text-gray-400 rounded-xl transition-colors bg-surface dark:bg-boxdark-2 hover:text-error active:scale-95">
        ...
        <div x-show="showEditModal" x-cloak
            ...
            <div class="relative w-full max-w-4xl bg-white dark:bg-boxdark rounded-[2rem] shadow-2xl border border-gray-100 dark:border-boxdark-2 p-6 md:p-8 pointer-events-auto max-h-[90vh] overflow-y-auto custom-scrollbar">
                ...
                <div class="flex justify-between items-center pb-4 mb-6 border-b border-gray-50 dark:border-boxdark-2">
                    <div><h3 class="text-xl font-black text-on-surface dark:text-white">تعديل بيانات الراكب</h3></div>
                    <button type="button" @click="closeModals()" class="flex justify-center items-center w-10 h-10 text-gray-400 rounded-xl transition-colors bg-surface dark:bg-boxdark-2 hover:text-error active:scale-95">
                        <span class="material-symbols-outlined">close</span>
                    </button>
                </div>
                ...
                <form :action="editPassengerData.url" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-5">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-2xl bg-gray-50/50 dark:bg-boxdark-2/50 border border-gray-100 dark:border-boxdark-2">
                            <div>
                                <label class="block mb-2 text-sm font-bold text-gray-600 dark:text-gray-300">التاريخ <span class="text-error">*</span></label>
                                <input type="date" name="date" x-model="editPassengerData.date" required class="px-4 w-full h-12 rounded-xl border-none ring-1 ring-gray-200 bg-white dark:bg-boxdark dark:text-white focus:ring-2 focus:ring-primary/40">
                            </div>
                            <div>
                                <label class="block mb-2 text-sm font-bold text-gray-600 dark:text-gray-300">المكان <span class="text-error">*</span></label>
                                <input type="text" name="location" x-model="editPassengerData.location" required class="px-4 w-full h-12 rounded-xl border-none ring-1 ring-gray-200 bg-white dark:bg-boxdark dark:text-white focus:ring-2 focus:ring-primary/40">
                            </div>
                            <div>
                                <label class="block mb-2 text-sm font-bold text-gray-600 dark:text-gray-300">الحالة <span class="text-error">*</span></label>
                                <select name="status" x-model="editPassengerData.status" required class="px-4 w-full h-12 rounded-xl border-none ring-1 ring-gray-200 bg-white dark:bg-boxdark dark:text-white focus:ring-2 focus:ring-primary/40">
                                    <option value="pending">قيد الانتظار</option>
                                    <option value="completed">مكتمل</option>
                                    <option value="cancel">ملغي</option>
                                </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="relative" x-data="passengerPhonePicker(@js(array_values(config('countries', []))))" x-effect="loadInitial(editPassengerData.passenger_number)">
                                <label class="block mb-2 text-sm font-bold text-gray-600 dark:text-gray-300">رقم الراكب <span class="text-error">*</span></label>
                                <input type="hidden" name="passenger_number" :value="fullPhoneNumber">
                                <div class="flex overflow-visible relative items-center bg-white rounded-xl ring-1 ring-gray-200 transition-all dark:bg-boxdark dark:ring-boxdark-2 focus-within:ring-2 focus-within:ring-primary/40" style="direction: ltr;">
                                    <div class="relative h-full" @click.away="openCountryDropdown = false">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block mb-2 text-sm font-bold text-gray-600 dark:text-gray-300">عدد الركاب <span class="text-error">*</span></label>
                                <input type="number" name="count" min="1" x-model="editPassengerData.count" required class="px-4 w-full h-12 rounded-xl border-none ring-1 ring-gray-200 bg-surface dark:bg-boxdark-2 dark:text-white focus:ring-2 focus:ring-primary/40">
                            </div>
                            <div>
                                <label class="block mb-2 text-sm font-bold text-gray-600 dark:text-gray-300">إجمالي العمولة <span class="text-error">*</span></label>
                                <input type="number" name="total_commission" min="0" step="0.01" x-model="editPassengerData.total_commission" required class="px-4 w-full h-12 rounded-xl border-none ring-1 ring-gray-200 bg-surface dark:bg-boxdark-2 dark:text-white focus:ring-2 focus:ring-primary/40 text-amber-600 font-black">
                            </div>
                        </div>

                        <div>
                            <label class="block mb-2 text-sm font-bold text-gray-600 dark:text-gray-300">ملاحظات</label>
                            <textarea name="note" rows="2" x-model="editPassengerData.note" class="px-4 py-3 w-full rounded-xl border-none ring-1 ring-gray-200 bg-surface dark:bg-boxdark-2 dark:text-white focus:ring-2 focus:ring-primary/40"></textarea>
                        </div>
                    </div>
                    <div class="pt-2">
                        <button type="submit" class="flex gap-2 justify-center items-center w-full h-12 text-sm font-black text-white rounded-xl shadow-lg transition-all bg-primary hover:bg-primary-hover active:scale-95">
        ...
        <div x-show="showStatusModal" x-cloak class="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6 pointer-events-none">
            <div class="fixed inset-0 backdrop-blur-sm pointer-events-auto bg-slate-900/60 dark:bg-black/80" @click="closeModals()"></div>

            <div class="relative w-full max-w-md bg-white dark:bg-boxdark rounded-[2rem] shadow-2xl border border-gray-100 dark:border-boxdark-2 p-6 md:p-8 pointer-events-auto text-center">
                <div class="flex justify-between items-center pb-4 mb-6 border-b border-gray-50 dark:border-boxdark-2 text-right">
                    <h3 class="text-xl font-black text-on-surface dark:text-white">تعيين حالة الراكب</h3>
                    <button type="button" @click="closeModals()" class="flex justify-center items-center w-8 h-8 text-gray-400 rounded-xl transition-colors bg-surface dark:bg-boxdark-2 hover:text-error active:scale-95">
                        <span class="material-symbols-outlined">close</span>
                    </button>
                </div>
                ...
                <form :action="statusPassengerData.url" method="POST" class="space-y-6 text-right">
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-bold text-gray-600 dark:text-gray-300">تحديث الحالة إلى:</label>
                        <select name="status" x-model="statusPassengerData.status" required class="px-4 w-full h-12 rounded-xl border-none ring-1 ring-gray-200 bg-white dark:bg-boxdark dark:text-white focus:ring-2 focus:ring-primary/40 text-sm font-bold">
                            <option value="pending">قيد الانتظار</option>
                            <option value="completed">مكتمل</option>
                            <option value="cancel">ملغي</option>
                        </select>
                    ...
                    <div class="flex gap-3 w-full pt-2">
                        <button type="button" @click="closeModals()" class="flex-1 h-12 text-sm font-bold text-gray-600 rounded-xl transition-all bg-surface dark:bg-boxdark-2 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-boxdark active:scale-95">إلغاء</button>
                        <button type="submit" class="flex-1 h-12 text-sm font-bold text-white rounded-xl shadow-lg transition-all bg-emerald-500 hover:bg-emerald-600 shadow-emerald-500/30 active:scale-95">حفظ الحالة</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        {{-- ====================== Delete Modal ====================== --}}
        <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6 pointer-events-none">
            <div class="fixed inset-0 backdrop-blur-sm pointer-events-auto bg-slate-900/60 dark:bg-black/80" @click="closeModals()"></div>
            ...
            <div class="relative w-full max-w-md bg-white dark:bg-boxdark rounded-[2rem] shadow-2xl border border-gray-100 dark:border-boxdark-2 p-8 text-center pointer-events-auto">
                ...
                <p class="mb-8 text-sm font-semibold leading-relaxed text-gray-500 dark:text-gray-400">
                    ...
                    <span class="text-base font-bold text-on-surface dark:text-white" x-text="deletePassengerData.passenger_number"></span>؟<br>
                ...
                <form :action="deletePassengerData.url" method="POST" class="flex gap-3 w-full">
                    @csrf
                    @method('DELETE')
                    <button type="button" @click="closeModals()" class="flex-1 h-12 text-sm font-bold text-gray-600 rounded-xl transition-all bg-surface dark:bg-boxdark-2 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-boxdark active:scale-95">تراجع</button>
                    <button type="submit" class="flex-1 h-12 text-sm font-bold text-white rounded-xl shadow-lg transition-all bg-error hover:bg-error/90 shadow-error/30 active:scale-95">نعم، احذف</button>
                </form>
            </div>
        </div>
    ...
    <script>
        function passengerRegistry() {
            return {
                ...
                editPassengerData: {
                    id: '', date: '', passenger_number: '', status: 'pending', location: '', count: 1, total_commission: 0,
                    customer_id: '', customer_name: '', customer_phone: '', driver_id: '', driver_name: '', driver_phone: '', note: '', url: ''
                },
                deletePassengerData: { id: '', passenger_number: '', url: '' },
                statusPassengerData: { 
                    id: '', status: 'pending', url: '', date: '', passenger_number: '', location: '', count: 1, total_commission: 0, 
                    customer_phone: '', customer_name: '', driver_phone: '', driver_name: '', note: ''
                },
                ...
                openEditModal(passenger) {
                    this.editPassengerData = { ...passenger, url: '{{ route('passengers.index') }}/' + passenger.id };
                    this.showEditModal = true;
                },

                openDeleteModal(id, passengerNumber) {
                    this.deletePassengerData = { id: id, passenger_number: passengerNumber, url: '{{ route('passengers.index') }}/' + id };
                    this.showDeleteModal = true;
                },

                openStatusModal(passenger) {
                    // نمرر كل البيانات كمدخلات مخفية حتى ينجح الـ Validation في الكنترولر بدون 404
                    this.statusPassengerData = { ...passenger, url: '{{ route('passengers.index') }}/' + passenger.id };
                    this.showStatusModal = true;
                },

                closeModals() {
                    this.showCreateModal = false; this.showEditModal = false; this.showDeleteModal = false; this.showStatusModal = false;
                },
                ...
                showRow(passengerNumber, location, driverName, customerName, statusKey) {
                    ...
                    const statusMap = { 'pending': 'قيد الانتظار', 'completed': 'مكتمل', 'cancel': 'ملغي' };
                    const arabicStatus = statusMap[statusKey] || statusKey;

                    const matchesSearch = !query || check(passengerNumber) || check(location) || check(driverName) || check(customerName) || check(arabicStatus);
                ...
                updateVisibility() {
                    this.$nextTick(() => { this.visibleCount = document.querySelectorAll('.passenger-row:not([style*="display: none"])').length; });
                }
            }
        }
        ...
        function recordPhonePicker(recordsList, countriesList) {
            return {
                ...
                resetSelection() { this.selectedRecordId = ''; this.nameInput = ''; },
                applyPhone(phone) {
                    const clean = String(phone || '').replace(/[^\d]/g, '');
                    const sortedCountries = [...this.countries].sort((a, b) => String(b.dial_code || '').length - String(a.dial_code || '').length);
                    const country = sortedCountries.find(c => {
