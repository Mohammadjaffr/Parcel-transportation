...
@section('content')
        ...
        {{-- 🌟 تم إضافة الشرط هنا: لن تعرض بطاقة المرسل بأكملها إلا إذا كان موجوداً 🌟 --}}
        @if($shipment->senderCustomer)
            ...
            <div class="flex flex-col p-6 bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:bg-boxdark dark:border-boxdark-2">
                ...
                <div class="flex relative z-10 flex-col flex-1 justify-between p-4 rounded-2xl border bg-slate-50/50 border-slate-100 dark:bg-boxdark-2/50 dark:border-boxdark">
                    <div>
                        <p class="text-base font-black text-slate-800 dark:text-white">{{ $shipment->senderCustomer->name }}</p>
                    ...
                    {{-- الأزرار الخاصة بالمرسل (مغلقة حالياً كما أرسلتها) --}}
                    {{-- @if($shipment->senderCustomer->phone)
                        <div class="flex gap-3 pt-5 mt-5 border-t border-slate-200 dark:border-boxdark">
                            <a href="https://wa.me/{{ preg_replace('/\D/', '', $shipment->senderCustomer->phone) }}?text={{ urlencode($whatsappMsg) }}" target="_blank"
        ...
        {{-- بطاقة المُستلم --}}
        <div class="flex flex-col p-6 bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:bg-boxdark dark:border-boxdark-2 relative overflow-hidden group {{ !$shipment->senderCustomer ? 'xl:col-span-2' : '' }}">
            {{-- 💡 ملاحظة: أضفت كلاس ديناميكي بالسطر الأعلى لكي تتمدد بطاقة المستلم بكامل العرض إذا كانت بطاقة المرسل مخفية --}}
            ...
            <div class="flex relative z-10 flex-col flex-1 justify-between p-4 rounded-2xl border bg-slate-50/50 border-slate-100 dark:bg-boxdark-2/50 dark:border-boxdark">
                <div>
                    ...
                    <p class="mt-1 text-sm font-bold text-right text-slate-500 dir-ltr dark:text-slate-400">
                        <x-phone-number :value="$shipment->receiverCustomer->phone ?? 'لا يوجد رقم'" />
                    </p>
                </div>
    ...
    <div class="p-6 bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:bg-boxdark dark:border-boxdark-2">
        ...
        @if($shipment->amount_to_collect_from_receiver == 0)
            <div class="flex justify-between items-center p-5 bg-emerald-50 rounded-2xl border border-emerald-100 dark:bg-emerald-500/10 dark:border-emerald-500/20">
                <div class="flex gap-3 items-center">
                    ...
                    <div>
                        <p class="text-[10px] font-black text-emerald-600/80 dark:text-emerald-400">حالة التحصيل</p>
                        <p class="text-base font-black text-emerald-700 dark:text-emerald-300">خالص / لا يوجد تحصيل</p>
                    </div>
                </div>
        ...
        @else
            {{-- 💡 وهنا نعرض المبلغ المتبقي بناءً على الدالة الجديدة --}}
            <div class="flex justify-between items-center p-5 bg-rose-50 rounded-2xl border border-rose-100 dark:bg-rose-500/10 dark:border-rose-500/20">
                <div class="flex gap-3 items-center">
                    ...
                    <div>
                        <p class="text-[10px] font-black text-rose-600/80 dark:text-rose-400">المبلغ المطلوب تحصيله من المستلم</p>
