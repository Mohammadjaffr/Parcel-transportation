...
<div class="pb-24 min-h-screen bg-surface dark:bg-boxdark-2 font-body lg:pb-12" dir="rtl" x-data="{ paymentMethod: 'prepaid', isSubmitting: false }">
    ...
    <div class="sticky top-0 z-40 border-b border-gray-100 shadow-sm backdrop-blur-md bg-white/90 dark:bg-boxdark/90 dark:border-boxdark-2">
        <div class="flex justify-between items-center px-4 py-4 mx-auto max-w-7xl md:px-6">
            <div class="flex gap-4 items-center">
                ...
                <div>
                    <h1 class="text-xl font-black md:text-2xl font-headline text-on-surface dark:text-white">إرسال طرد</h1>
    ...
    <div class="p-4 mx-auto mt-4 max-w-7xl md:p-6">
        ...
        <form id="shipmentForm" action="{{ route('shipment.store') }}" method="POST" @submit="if(!isSubmitting) { isSubmitting = true; return true; } else { $event.preventDefault(); return false; }" class="grid grid-cols-1 gap-6 items-start lg:grid-cols-12">
            @csrf
            ...
            <div class="flex flex-col gap-6 lg:col-span-7 xl:col-span-8">
                ...
                <div x-data='destinationLogic(@json($offices))' class="bg-white dark:bg-boxdark p-6 md:p-8 rounded-[2rem] border border-gray-100 dark:border-boxdark-2 shadow-sm relative overflow-hidden group/card">
                    ...
                    <div class="relative z-10">
                        <div class="flex gap-3 items-center mb-6">
                            <div class="flex justify-center items-center w-12 h-12 text-gray-500 rounded-xl shadow-sm transition-all shrink-0 bg-surface dark:bg-boxdark-2 dark:text-bodydark group-hover/card:bg-primary group-hover/card:text-white">
                                <span class="material-symbols-outlined text-[24px]">local_shipping</span>
                            </div>
                            <div>
                                <h3 class="text-lg font-black text-on-surface dark:text-white font-headline">وجهة الطرد</h3>
                                <p class="mt-1 text-xs font-medium text-gray-500 dark:text-bodydark">اختر المكتب والفرع المستلم</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6">
                            {{-- اختيار المكتب --}}
                            <div>
                                <label class="block mb-2 text-xs font-bold text-gray-600 dark:text-gray-300">المكتب <span class="text-error">*</span></label>
                                <div class="relative">
                            ...
                            <div x-show="selectedOfficeId" x-transition:enter.duration.300ms x-cloak>
                                <label class="block mb-2 text-xs font-bold text-gray-600 dark:text-gray-300">الفرع <span class="text-error">*</span></label>
                                <div class="relative">
                ...
                {{-- 2. بيانات العملاء --}}
                <div class="bg-white dark:bg-boxdark p-6 md:p-8 rounded-[2rem] border border-gray-100 dark:border-boxdark-2 shadow-sm">
                    <h3 class="flex gap-2 items-center mb-6 text-lg font-black text-on-surface dark:text-white font-headline">
                        <div class="flex justify-center items-center w-10 h-10 text-green-500 bg-green-50 rounded-lg dark:bg-green-500/10 shrink-0">
                    ...
                    <div class="space-y-8">
                        ...
                        <div x-data="customerSelect({{ $customers }}, @js(array_values(config('countries', []))))" class="relative z-50">
                            <h4 class="pb-2 mb-3 text-sm font-bold text-gray-700 border-b border-gray-100 dark:text-gray-200 dark:border-boxdark-2">بيانات المرسل</h4>
                            ...
                            <div class="grid relative grid-cols-1 gap-4 md:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <div class="flex overflow-visible relative items-center bg-white rounded-xl ring-1 ring-gray-200 transition-all group dark:bg-boxdark dark:ring-boxdark-2 focus-within:ring-2 focus-within:ring-primary/40 focus-within:border-primary"
                                        :class="selectedCustomerId ? 'bg-primary-container dark:bg-primary/10 ring-primary/30' : ''" style="direction: ltr;">
                                            ...
                                            <div x-show="openCountryDropdown" x-cloak x-transition
                                                ...
                                                <div class="overflow-y-auto max-h-48 custom-scrollbar" dir="ltr">
                                                    <template x-for="country in filteredCountries" :key="country.code">
                                                        <button type="button" @click="selectedCountry = country; openCountryDropdown = false; searchCustomer()"
                                        ...
                                        <input type="tel" x-model="localPhoneNumber" @input="searchCustomer" @focus="showCustomerDropdown = true" @click.away="showCustomerDropdown = false"
                                            placeholder="7XXXXXXXX" required inputmode="numeric" autocomplete="off" :maxlength="selectedCountry?.code === 'YE' ? 9 : 15"
                                        ...
                                        <button type="button" x-show="selectedCustomerId" @click="resetSelection" class="absolute right-2 z-10 p-1 text-gray-400 rounded-full transition-colors bg-white/80 dark:bg-boxdark/80 hover:text-error">
                                            <span class="material-symbols-outlined text-[16px]">close</span>
                                        </button>
                                    </div>

                                    {{-- Customer Search Dropdown --}}
                                    <div x-show="showCustomerDropdown && localPhoneNumber.length > 0 && !selectedCustomerId" x-transition x-cloak
                                        ...
                                        <div x-show="filteredCustomers.length === 0" class="px-4 py-3 text-center bg-surface dark:bg-boxdark-2">
                                            <span class="text-xs font-bold text-gray-500 dark:text-bodydark">مرسل جديد، سيتم حفظه تلقائياً.</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {{-- الاسم --}}
                                <div>
                                    <label class="block mb-1.5 text-xs font-bold text-gray-500 dark:text-gray-400">اسم المرسل (اختياري)</label>
                                    <input type="text" name="sender_name" x-model="nameInput" :readonly="selectedCustomerId !== null" placeholder="الاسم..."
                                        ...
                                        :class="selectedCustomerId ? 'bg-surface border-transparent text-gray-500 cursor-not-allowed opacity-80' : 'bg-white border-gray-200 focus:border-primary focus:ring-2 focus:ring-primary/20'">
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-data="customerSelect({{ $customers }}, @js(array_values(config('countries', []))))" class="relative z-40">
                            <h4 class="pb-2 mb-3 text-sm font-bold text-gray-700 border-b border-gray-100 dark:text-gray-200 dark:border-boxdark-2">بيانات المستلم</h4>
                            ...
                            <div class="grid relative grid-cols-1 gap-4 md:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <div class="flex overflow-visible relative items-center bg-white rounded-xl ring-1 ring-gray-200 transition-all group dark:bg-boxdark dark:ring-boxdark-2 focus-within:ring-2 focus-within:ring-green-500/40 focus-within:border-green-500"
                                        :class="selectedCustomerId ? 'bg-green-50/30 dark:bg-green-500/10 ring-green-400 dark:ring-green-500/50' : ''" style="direction: ltr;">
                                        ...
                                        <div class="relative h-full" @click.away="openCountryDropdown = false">
                                            ...
                                            <div x-show="openCountryDropdown" x-cloak x-transition
                                                ...
                                                <div class="p-2 border-b border-gray-50 dark:border-boxdark">
                                                    <input type="text" x-model="searchCountryQuery" placeholder="بحث..."
                                                        class="px-3 w-full h-9 text-xs rounded-lg outline-none bg-surface dark:bg-boxdark focus:ring-1 ring-green-500/30 text-on-surface dark:text-white" dir="rtl">
                                                </div>
                                                <div class="overflow-y-auto max-h-48 custom-scrollbar" dir="ltr">
                                                    <template x-for="country in filteredCountries" :key="country.code">
                                                        <button type="button" @click="selectedCountry = country; openCountryDropdown = false; searchCustomer()"
                                    ...
                                    <div x-show="showCustomerDropdown && localPhoneNumber.length > 0 && !selectedCustomerId" x-transition x-cloak
                                        ...
                                        <div x-show="filteredCustomers.length === 0" class="px-4 py-3 text-center bg-surface dark:bg-boxdark-2">
                                            <span class="text-xs font-bold text-gray-500 dark:text-bodydark">مستلم جديد، سيتم حفظه تلقائياً.</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {{-- الاسم --}}
                                <div>
                                    <label class="block mb-1.5 text-xs font-bold text-gray-500 dark:text-gray-400">اسم المستلم <span class="text-error">*</span></label>
                                    <input type="text" name="receiver_name" required x-model="nameInput" :readonly="selectedCustomerId !== null" placeholder="الاسم..."
                                        class="px-4 w-full h-12 text-sm rounded-xl border transition-colors outline-none dark:bg-boxdark-2 dark:border-boxdark dark:text-white"
                                        :class="selectedCustomerId ? 'bg-surface border-transparent text-gray-500 cursor-not-allowed opacity-80' : 'bg-white border-gray-200 focus:border-green-500 focus:ring-2 focus:ring-green-500/20'">
                                </div>
                            </div>
                        </div>
            ...
            <div class="flex flex-col gap-6 lg:col-span-5 xl:col-span-4">
                ...
                <div class="bg-white dark:bg-boxdark p-6 rounded-[2rem] border border-gray-100 dark:border-boxdark-2 shadow-sm">
                    <h3 class="flex gap-2 items-center mb-6 text-lg font-black text-on-surface dark:text-white font-headline">
                        <div class="flex justify-center items-center w-10 h-10 text-blue-500 bg-blue-50 rounded-lg dark:bg-blue-500/10 shrink-0">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="package_type" class="px-3 w-full h-12 text-sm rounded-xl border border-gray-200 transition-all appearance-none outline-none bg-surface dark:bg-boxdark-2 dark:border-boxdark focus:border-primary focus:ring-2 focus:ring-primary/20 text-on-surface dark:text-white">
                        ...
                        <div>
                            <label class="block mb-1.5 text-xs font-bold text-gray-600 dark:text-gray-300">الوزن (كجم)</label>
                            <input type="number" step="0.1" name="weight" placeholder="مثال: 2.5"
                                class="px-4 w-full h-12 text-sm text-left rounded-xl border border-gray-200 transition-all outline-none bg-surface dark:bg-boxdark-2 dark:border-boxdark focus:border-primary focus:ring-2 focus:ring-primary/20 text-on-surface dark:text-white" dir="ltr">
                        </div>
                    </div>

                    <div class="grid grid-cols-2 gap-4 p-4 mt-5 rounded-2xl border bg-amber-50/50 dark:bg-amber-500/5 border-amber-100/50 dark:border-amber-500/10">
                        <div>
                            <label class="block text-[10px] font-bold text-amber-600 dark:text-amber-500 mb-1.5">دباب العسل</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-bold text-amber-600 dark:text-amber-500 mb-1.5">قروف العسل</label>
                    ...
                    <div class="mt-5">
                        <label class="block mb-1.5 text-xs font-bold text-gray-600 dark:text-gray-300">ملاحظات إضافية</label>
                ...
                <div class="bg-white dark:bg-boxdark p-6 rounded-[2rem] border border-gray-100 dark:border-boxdark-2 shadow-sm">
                    <h3 class="flex gap-2 items-center mb-6 text-lg font-black text-on-surface dark:text-white font-headline">
                        <div class="flex justify-center items-center w-10 h-10 text-rose-500 bg-rose-50 rounded-lg dark:bg-rose-500/10 shrink-0">
                    ...
                    <div class="space-y-5">
                        <div>
                            <label class="block mb-1.5 text-xs font-bold text-gray-600 dark:text-gray-300">طريقة الدفع <span class="text-error">*</span></label>
                        ...
                        <div>
                            <label class="block mb-1.5 text-xs font-bold text-gray-600 dark:text-gray-300">المبلغ الإجمالي (ريال) <span class="text-error">*</span></label>
                            <input type="number" required name="total_amount" placeholder="0.00"
                                class="px-4 w-full h-14 text-xl font-black text-left rounded-2xl border border-gray-200 transition-all outline-none bg-surface dark:bg-boxdark-2 dark:border-boxdark focus:border-primary focus:ring-2 focus:ring-primary/20 text-primary dark:text-primary" dir="ltr">
                        </div>

                        <div x-show="paymentMethod === 'partial_payment'" x-collapse>
                            <label class="block text-[11px] font-bold text-rose-500 dark:text-rose-400 mb-1.5">المبلغ المدفوع حالياً (ريال)</label>
                            <input type="number" name="partial_amount" placeholder="0.00"
                                class="px-4 w-full h-14 text-xl font-black text-left text-rose-600 rounded-2xl border border-rose-200 transition-all outline-none bg-rose-50/50 dark:bg-rose-500/5 dark:border-rose-500/20 focus:border-rose-400 dark:text-rose-400" dir="ltr">
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-2 md:hidden">
                    <button type="submit" :disabled="isSubmitting"
                        ...
                        :class="isSubmitting ? 'bg-gray-400 dark:bg-gray-600 text-white cursor-not-allowed shadow-none' : 'bg-primary text-white hover:bg-primary-hover shadow-primary/30 active:scale-95'">
                        <span x-show="!isSubmitting" class="material-symbols-outlined text-[22px]">send</span>
                        <span x-show="isSubmitting" class="animate-spin material-symbols-outlined text-[22px]">progress_activity</span>
                        <span x-text="isSubmitting ? 'جاري الاعتماد...' : 'اعتماد وإصدار السند'"></span>
                    </button>
                </div>
            </div>
