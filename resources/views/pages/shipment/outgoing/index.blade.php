...
@section('content')
    ...
    <div class="pb-24 space-y-6 min-h-screen font-body lg:pb-12"
         dir="rtl"
         x-data="outgoingShipmentsRegistry()">
        ...
        <div class="bg-white dark:bg-boxdark my-4 rounded-[2rem] border border-gray-100 dark:border-boxdark-2 shadow-sm overflow-visible transition-colors max-w-7xl mx-auto">
            ...
            <div class="flex flex-col gap-4 p-5 lg:hidden">
                @forelse($shipments as $shipment)
                    @php
                        ...
                        if ($shipment->receiverOfficeBranch) {
                            $destination = ($shipment->receiverOfficeBranch->office->name ?? 'مكتب خارجي') . ' - ' . $shipment->receiverOfficeBranch->name;
                            ...
                            if ($shipment->senderBranch?->app_id == $shipment->receiverBranch->app_id) {
                                ...
                                $destination = ($shipment->receiverBranch->app->name ?? 'مكتب موثوق') . ' - ' . $shipment->receiverBranch->name;
                            }
                        }
                ...
                <div x-show="visibleCount === 0 && {{ $shipments->count() }} > 0"
                    x-cloak
                            ...
                            <tr class="transition-colors hover:bg-gray-50/80 dark:hover:bg-boxdark-2/50 group shipment-row"
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex flex-col gap-1.5 items-center">
                                        ...
                                        @if ($shipment->no_gallons_honey > 0 || $shipment->no_honey_jars > 0)
                                            <span class="flex items-center gap-1 text-[10px] font-black text-amber-600 dark:text-amber-400">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex flex-col gap-1 items-center">
                                        ...
                                        <span class="px-2.5 py-1 text-[10px] font-black rounded-lg {{ $shipment->payment_method == 'prepaid' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400' }}">
                                            {{ $paymentLabel }}
                                        </span>
    ...
    <script>
        function outgoingShipmentsRegistry() {
            return {
                ...
                updateVisibility() {
                    this.$nextTick(() => {
                        this.visibleCount = document.querySelectorAll('.shipment-row:not([style*="display: none"])').length;
                    });
                }
            }
        }
