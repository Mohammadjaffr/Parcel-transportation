    ...
    <div class="max-w-xl mx-auto bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
        ...
        <form action="{{ route('shipments.storeCustomer') }}" method="POST" class="space-y-6"
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" required class="w-full h-11 rounded-lg border border-gray-300 dark:border-gray-600
                           bg-white dark:bg-gray-900 px-4 text-sm
                           text-gray-700 dark:text-gray-200
                           focus:ring-2 focus:ring-brand-500 focus:border-brand-500" placeholder="اسم العميل">
                @error('name')
            ...
            <div>
                ...
                <input type="text" name="phone" value="{{ old('phone') }}" required class="w-full h-11 rounded-lg border border-gray-300 dark:border-gray-600
                           bg-white dark:bg-gray-900 px-4 text-sm
                           text-gray-700 dark:text-gray-200
                           focus:ring-2 focus:ring-brand-500 focus:border-brand-500" placeholder="رقم الهاتف">
                @error('phone')
                    <div class="text-sm text-error-600 mt-1">{{ $message }}</div>
                @enderror
            </div>
            ...
            {{-- زر الحفظ --}}
            <div class="pt-4">
                <button type="submit" :disabled="isSubmitting" class="flex justify-center items-center gap-2 w-full bg-brand-500 hover:bg-brand-600
                           text-white font-medium py-2.5 rounded-lg disabled:opacity-75 disabled:cursor-not-allowed">
                    <span x-show="!isSubmitting">حفظ والمتابعة لإنشاء الطرد</span>
