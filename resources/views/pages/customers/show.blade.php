...
@section('content')
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <div class="flex items-center gap-4 mt-2">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm font-medium text-gray-800 dark:text-gray-200">{{ number_format($customer->credit_limit, 2) }} ر.س</span>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('customers.edit', $customer->id) }}"
                    class="px-4 py-2 bg-yellow-50 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 
                           border border-yellow-200 dark:border-yellow-800 rounded-lg hover:bg-yellow-100 
                           dark:hover:bg-yellow-900/50 transition-colors text-sm">
                    تعديل العميل
                </a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-800 dark:text-white">
                            {{ number_format($debit, 2) }} <span class="text-sm">ر.س</span>
                        </p>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-5 shadow-sm">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-800 dark:text-white">
                            {{ number_format($credit, 2) }} <span class="text-sm">ر.س</span>
                        </p>
                    </div>
            ...
            <!-- الرصيد -->
            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-5 shadow-sm 
                {{ $balance > 0 ? 'border-red-200 dark:border-red-800' : 'border-green-200 dark:border-green-800' }}">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-1">الرصيد الحالي</p>
                        <p class="text-2xl font-bold {{ $balance > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400' }}">
                            {{ number_format(abs($balance), 2) }} <span class="text-sm">ر.س</span>
                        </p>
                    </div>
                    <div class="w-12 h-12 rounded-full {{ $balance > 0 ? 'bg-red-50 dark:bg-red-900/20' : 'bg-green-50 dark:bg-green-900/20' }} 
                ...
                <p class="text-xs {{ $balance > 0 ? 'text-red-500 dark:text-red-400' : 'text-green-500 dark:text-green-400' }} mt-3 font-medium">
                    {{ $balance > 0 ? 'مدين' : 'دائن' }}
                </p>
            </div>
        </div>
                ...
                <table class="w-full text-sm text-right">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            ...
                            <!-- المبلغ -->
                            <td class="p-4 font-medium {{ $t->type === 'debit' ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400' }}">
