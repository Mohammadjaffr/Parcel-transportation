    ...
    <div class="space-y-6 font-outfit" dir="rtl" x-data="customerRegistry()">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-6">
            ...
            <div @click="filterStatus = 'debtor'" :class="filterStatus === 'debtor' ? 'border-error-500 ring-2 ring-error-500/20' :
                ...
                <div class="mt-3">
                    <span class="text-theme-xs text-error-500 font-bold uppercase tracking-widest">المديونين</span>
            ...
            <div @click="filterStatus = 'cleared'" :class="filterStatus === 'cleared' ? 'border-success-500 ring-2 ring-success-500/20' :
                ...
                <div class="mt-3">
                    <span class="text-theme-xs text-success-500 font-bold uppercase tracking-widest">رصيد خالص</span>
                    <h4 class="text-xl font-black dark:text-white">
                        {{ $customers->getCollection()->filter(fn($c) => ($c->debit_sum ?? 0) <= ($c->credit_sum ?? 0))->count() }}
                    </h4>
                </div>
        ...
        <div
            ...
            <div class="relative group w-full">
                <input type="text" x-model="search" placeholder="ابحث باسم العميل أو رقم الهاتف..."
                    class="w-full h-12 pr-11 pl-4 rounded-xl border-none bg-gray-50 dark:bg-gray-900 focus:ring-2 focus:ring-brand-500/20 transition-all text-sm font-medium dark:text-white placeholder-gray-400 shadow-inner">
                <div
                    class="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 group-focus-within:text-brand-500 transition-colors">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-24 text-center">
                                    <div class="text-gray-400 font-bold italic">لا توجد بيانات عملاء مطابقة للبحث..</div>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
