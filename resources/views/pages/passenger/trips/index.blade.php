    ...
    <div class="mx-auto w-full max-w-7xl">
        <div class="flex gap-4 justify-between items-start">
            ...
            <div class="flex gap-2 items-center shrink-0">
                <a href="{{ route('trips.create') }}" class="inline-flex gap-2.5 items-center px-5 h-12 text-sm font-black text-white rounded-2xl transition-all bg-primary hover:bg-primary-hover hover:shadow-lg hover:shadow-primary/25 active:scale-95">
    ...
    <div class="bg-white dark:bg-boxdark my-4 rounded-2xl border border-gray-100 dark:border-boxdark-2 shadow-sm overflow-visible transition-colors max-w-7xl mx-auto">
        ...
        <div class="p-5 w-full border-b border-gray-100 md:p-6 dark:border-boxdark-2">
            <div class="flex flex-col gap-4 justify-between items-stretch md:flex-row md:items-center">
                <div class="relative flex flex-row items-center px-3 w-full gap-3 rounded-2xl border border-gray-200 transition-all md:w-[420px] dark:border-boxdark-2 group focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 bg-slate-50 dark:bg-boxdark-2">
                    <input type="text" x-model="searchQuery" @input.debounce.300ms="updateVisibility()" placeholder="ابحث برقم أو اسم السائق..."
            ...
            <table class="table-auto w-full text-right border-collapse">
                <thead>
                    <tr class="bg-slate-50/80 dark:bg-boxdark-2 border-b border-gray-100 dark:border-boxdark-2">
                        <th class="px-6 py-4 text-[11px] font-black text-gray-400 uppercase tracking-wider font-headline">الرحلة والتاريخ</th>
                        <th class="px-6 py-4 text-[11px] font-black text-gray-400 uppercase tracking-wider font-headline">بيانات السائق</th>
                        <th class="px-6 py-4 text-[11px] font-black text-gray-400 uppercase tracking-wider font-headline text-center">إجمالي الركاب</th>
                        <th class="px-6 py-4 text-[11px] font-black text-gray-400 uppercase tracking-wider font-headline text-center">الإجراءات</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-boxdark-2">
                    @forelse($trips as $trip)
                        @php
                            ...
                            if(empty($avatarLetters)) $avatarLetters = 'س';
                        @endphp
                        <tr class="transition-colors hover:bg-slate-50/80 dark:hover:bg-boxdark-2/50 group trip-row"
                            ...
                            <td class="px-6 py-4 align-top">
                                <div class="flex flex-col gap-1">
                                    <span class="font-body text-sm font-bold text-gray-800 dark:text-white">رحلة #{{ $trip->id }}</span>
                                    <span class="font-body text-xs text-gray-500 dark:text-gray-400">{{ $trip->created_at->format('Y-m-d h:i A') }}</span>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 align-top">
                                <div class="flex items-center gap-3">
                                    <div class="flex items-center justify-center w-11 h-11 rounded-full bg-primary/10 text-primary font-headline font-black uppercase shadow-sm shrink-0">
                                        {{ $avatarLetters }}
                                    </div>
                                    <div class="flex flex-col gap-1">
                                        ...
                                        @if($driverPhone)
                                            <span class="font-body text-xs text-gray-500 dark:text-gray-400 dir-ltr text-right font-mono block mt-0.5" style="direction: ltr;">
                                                <x-phone-number :value="$driverPhone" class="text-gray-500 dark:text-gray-400" />
                                            </span>
                                        @else
                                            <span class="text-[10px] text-amber-500 font-bold block mt-0.5">يرجى تعيين سائق للرحلة</span>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 align-middle text-center">
                                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-black bg-slate-100 text-slate-700 dark:bg-boxdark-2 dark:text-gray-300">
                        ...
                        <tr>
                            <td colspan="4" class="py-24 text-center">
                                <div class="flex flex-col gap-4 justify-center items-center">
                                    <div class="flex justify-center items-center w-16 h-16 bg-slate-50 rounded-full border border-gray-100 dark:bg-boxdark-2 dark:border-boxdark text-gray-400">
                                        <span class="material-symbols-outlined text-[32px]">explore_off</span>
                                    </div>
                                    <div>
                                        <h3 class="mb-1 font-headline text-base font-black text-slate-700 dark:text-white">لا توجد رحلات مضافة حالياً</h3>
                                        <p class="font-body text-sm font-medium text-gray-500 dark:text-gray-400">قم بإنشاء أول رحلة الآن لتبدأ بتوزيع الركاب بداخلها.</p>
                                    </div>
                                    <a href="{{ route('trips.create') }}" class="mt-2 px-6 h-12 bg-primary text-white text-sm font-black rounded-xl shadow-md shadow-primary/20 flex items-center gap-2 transition-all active:scale-95">
                    ...
                    <tr x-show="visibleCount === 0 && {{ $trips->count() }} > 0" x-cloak>
                        <td colspan="4" class="py-24 text-center">
                            <div class="flex flex-col gap-4 justify-center items-center">
                                ...
                                <div>
                                    <h3 class="mb-1 font-headline text-base font-black text-slate-700 dark:text-white">لا توجد نتائج مطابقة</h3>
                                    <p class="font-body text-sm font-medium text-gray-500 dark:text-gray-400">لم نعثر على رحلات تطابق بحثك.</p>
                                </div>
                            </div>
                        </td>
                    </tr>
        ...
        @if ($trips->hasPages())
            <div class="px-6 py-5 border-t border-gray-100 dark:border-boxdark-2 bg-slate-50/50 dark:bg-boxdark-2/50 rounded-b-2xl custom-pagination">
...
<script>
    function tripsData() {
        return {
            ...
            updateVisibility() {
                this.$nextTick(() => {
                    this.visibleCount = document.querySelectorAll('.trip-row:not([style*="display: none"])').length;
                });
            }
        }
    }
