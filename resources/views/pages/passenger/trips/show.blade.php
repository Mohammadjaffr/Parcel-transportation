            ...
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center gap-4">
                    <a href="{{ route('trips.index') }}"
                        class="flex justify-center items-center w-12 h-12 bg-white dark:bg-boxdark rounded-2xl border shadow-sm transition-all border-slate-100 dark:border-boxdark-2 text-slate-500 dark:text-gray-400 hover:text-primary hover:border-primary/30 active:scale-95">
                        <span class="material-symbols-outlined text-[24px]">arrow_forward</span>
                    </a>
                    <div>
                        <h1
                            class="text-2xl font-black font-headline text-slate-800 dark:text-white flex items-center gap-2">
                            تفاصيل الرحلة
                            <span
                                class="text-primary font-mono bg-primary/10 px-2 py-0.5 rounded-lg text-lg">#{{ $trip->id }}</span>
                        </h1>
                        <p class="text-sm font-bold text-gray-500 dark:text-gray-400 mt-1">تاريخ الإنشاء:
                ...
                <div class="flex gap-3">
                    <a href="{{ route('receipt.generate', ['type' => 'trip', 'id' => $trip->uuid]) }}" target="_blank"
                        class="flex items-center gap-2 px-5 h-12 bg-white dark:bg-boxdark text-emerald-600 dark:text-emerald-400 rounded-2xl border border-emerald-100 dark:border-emerald-500/20 shadow-sm transition-all hover:bg-emerald-50 dark:hover:bg-emerald-500/10 active:scale-95 font-bold text-sm">
                        <span class="material-symbols-outlined text-[20px]">print</span>
                        طباعة الكشف
                    </a>
                    <a href="{{ route('trips.edit', $trip->id) }}"
                        class="flex items-center gap-2 px-5 h-12 bg-white dark:bg-boxdark text-amber-600 dark:text-amber-400 rounded-2xl border border-amber-100 dark:border-amber-500/20 shadow-sm transition-all hover:bg-amber-50 dark:hover:bg-amber-500/10 active:scale-95 font-bold text-sm">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-1 space-y-6">
                    ...
                    <div
                        ...
                        <h3
                            class="text-sm font-black text-gray-500 dark:text-gray-400 mb-4 flex items-center gap-2 font-headline">
                        ...
                        <div class="flex items-center gap-4">
                            <div
                                class="w-16 h-16 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 flex items-center justify-center border border-emerald-100 dark:border-emerald-500/20 text-emerald-600 dark:text-emerald-400 shrink-0">
                            ...
                            <div class="min-w-0 flex-1">
                                ...
                                @if($trip->driver?->phone)
                                    <a href="tel:{{ $trip->driver->phone }}" class="inline-block mt-1.5"
                                        style="direction: ltr;">
                                        <div
                                            class="bg-primary/5 px-2.5 py-1 rounded-xl border border-primary/10 transition-colors hover:bg-primary/10">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div
                            class="p-5 bg-white dark:bg-boxdark rounded-[2rem] border border-gray-100 dark:border-boxdark-2 shadow-sm flex flex-col justify-center items-center gap-2 text-center">
                            <div
                                class="w-12 h-12 rounded-xl bg-amber-50 dark:bg-amber-500/10 text-amber-500 flex items-center justify-center">
                ...
                <div class="lg:col-span-2">
                    <div
                        ...
                        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
                            <div>
                                <h3
                                    class="font-black text-slate-800 dark:text-white font-headline text-lg flex items-center gap-2">
                                    <span class="material-symbols-outlined text-primary">group</span>
                                    ركاب الرحلة المقيدين
                                </h3>
                                <p class="text-xs text-gray-500 mt-1 font-bold">قائمة الركاب الذين تم تسييرهم في هذه الرحلة.
                                </p>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 content-start auto-rows-max overflow-y-auto custom-scrollbar pr-2"
                            ...
                            @forelse($trip->passengers as $passenger)
                                <div class="bg-slate-50/50 dark:bg-boxdark-2/50 p-4 rounded-2xl border border-slate-100 dark:border-boxdark-2 flex items-center justify-between gap-4 hover:border-primary/30 transition-colors"
                                    ...
                                    <div class="flex items-center gap-4 min-w-0 flex-1">
                                        <div
                                            class="flex flex-col justify-center items-center w-12 h-12 rounded-xl bg-white dark:bg-boxdark text-slate-600 dark:text-gray-300 shrink-0 border border-slate-200 dark:border-boxdark-2 shadow-sm">
                                            <span class="text-lg font-black leading-none">{{ $passenger->count ?? 1 }}</span>
                                            <span class="text-[9px] font-bold mt-0.5">ركاب</span>
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <span
                                                class="block font-headline text-base font-black text-slate-800 dark:text-white tracking-tight truncate mb-1"
                                            ...
                                            <div class="flex gap-1.5 items-center">
                                                ...
                                                <span
                                                    class="text-xs font-bold text-slate-500 dark:text-gray-400 truncate">{{ $passenger->destination ?? ' ' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-2 shrink-0">
                                        <form
                                            action="{{ route('trip.removePassenger', ['trip' => $trip->id, 'passenger' => $passenger->id]) }}"
                                            method="POST"
                                            onsubmit="return confirm('تأكيد فك ارتباط الراكب وإزالته من الرحلة؟');">
                                            @csrf

                                            <button type="submit"
                                                class="flex justify-center items-center w-8 h-8 text-gray-400 bg-white rounded-full border border-slate-200 shadow-sm transition-all dark:bg-boxdark hover:text-rose-500 hover:bg-rose-50 dark:border-boxdark-2 dark:hover:bg-rose-500/10 dark:hover:border-rose-500/20"
                                                title="فك ارتباط الراكب">
                            ...
                            @empty
                                <div
                                    class="col-span-1 md:col-span-2 py-12 text-center flex flex-col items-center justify-center text-slate-400 bg-slate-50 dark:bg-boxdark-2/30 rounded-2xl border border-dashed border-slate-200 dark:border-boxdark-2">
                                    <div
                                        class="flex justify-center items-center mb-3 w-16 h-16 rounded-full bg-white dark:bg-boxdark shadow-sm">
