...
<div class="space-y-6 font-body" dir="rtl">
    <div class="flex gap-4 items-center mb-6">
        <a href="{{ route('offices.unverified.index') }}"
            class="flex justify-center items-center w-10 h-10 text-gray-500 bg-white rounded-xl border border-gray-200 shadow-sm transition-all hover:bg-surface hover:text-primary dark:bg-boxdark-2 dark:border-boxdark dark:text-bodydark">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                {{-- سهم متوافق مع اتجاه RTL للعودة --}}
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3" />
            </svg>
        </a>
        <div>
            <h1 class="text-2xl font-black text-on-surface dark:text-white">تعديل بيانات المكتب</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-bodydark">تحديث معلومات مكتب <span class="font-bold text-primary">{{ $office->name }}</span> وفروعه.</p>
        </div>
    </div>
    ...
    <form action="{{ route('offices.update', $office->id) }}" method="POST" x-data="{ isSubmitting: false }" @submit="isSubmitting = true">
        ...
        <div class="space-y-6">
            ...
            <div class="overflow-hidden bg-white rounded-3xl border border-gray-200 shadow-sm dark:bg-boxdark dark:border-boxdark-2">
                <div class="flex gap-3 items-center px-6 py-5 border-b border-gray-100 bg-surface dark:bg-boxdark-2 dark:border-boxdark">
                        ...
                        <div>
                            ...
                            <div x-data="phoneComponent('{{ old('phone', $office->phone) }}')" class="relative">
                                ...
                                <div class="flex overflow-hidden w-full h-12 bg-white rounded-xl border border-gray-300 transition-all focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 dark:border-boxdark dark:bg-boxdark-2 dark:focus-within:border-primary">
                                    <button type="button" @click="open = !open" class="flex gap-2 items-center px-3 border-l border-gray-200 transition-colors bg-surface shrink-0 dark:bg-boxdark dark:border-boxdark hover:bg-gray-100 dark:hover:bg-gray-800">
            ...
            <div class="overflow-hidden bg-white rounded-3xl border border-gray-200 shadow-sm dark:bg-boxdark dark:border-boxdark-2" 
                ...
                <div class="flex justify-between items-center px-6 py-5 border-b border-gray-100 bg-surface dark:bg-boxdark-2 dark:border-boxdark">
                    <div class="flex gap-3 items-center">
                        ...
                        <h3 class="text-lg font-bold text-on-surface dark:text-white">تحديث الفروع <span class="text-error">*</span></h3>
                    </div>
                    <button type="button" @click="branches.push({ name: '', city: '', phone: '', address: '' })"
                        class="inline-flex gap-2 items-center px-5 py-2.5 text-sm font-bold rounded-xl transition-all text-primary-hover bg-primary-container hover:bg-primary/20 dark:bg-primary/10 dark:text-primary hover:scale-95">
                ...
                <div class="p-6 space-y-6 md:p-8 bg-surface dark:bg-boxdark-2/50">
                    <template x-for="(branch, index) in branches" :key="index">
                        <div class="relative p-6 bg-white rounded-2xl border border-gray-200 shadow-sm transition-all dark:bg-boxdark dark:border-boxdark hover:shadow-md hover:border-primary/30 dark:hover:border-primary/50">
                            
                            <button type="button" @click="branches.splice(index, 1)" x-show="branches.length > 1"
                                class="absolute top-4 left-4 p-2 text-gray-400 rounded-xl transition-all bg-surface hover:text-error hover:bg-error/10 dark:bg-boxdark-2 dark:hover:bg-error/20 dark:hover:text-error" title="حذف الفرع">
                            ...
                            <div class="flex gap-2 items-center mb-5">
                                <span class="flex justify-center items-center w-6 h-6 text-xs font-bold rounded-full text-primary-hover bg-primary-container dark:bg-primary/20 dark:text-primary" x-text="index + 1"></span>
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="text" x-model="branch.name" :name="`branches[${index}][name]`" required placeholder="مثال: الفرع الرئيسي"
                                            class="block pr-10 pl-4 w-full h-12 text-sm rounded-xl border-gray-300 transition-colors bg-surface focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 dark:bg-boxdark-2 dark:border-boxdark dark:text-white dark:focus:bg-boxdark-2 dark:focus:border-primary">
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div x-data="phoneComponent(branch.phone)" class="relative">
                                        ...
                                        <div class="flex overflow-hidden w-full h-12 rounded-xl border border-gray-300 transition-all bg-surface focus-within:bg-white focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 dark:border-boxdark dark:bg-boxdark-2 dark:focus-within:bg-boxdark-2 dark:focus-within:border-primary">
                                            <button type="button" @click="open = !open" class="flex gap-2 items-center px-3 border-l border-gray-200 transition-colors shrink-0 hover:bg-gray-100 dark:border-boxdark dark:hover:bg-boxdark">
                                                <template x-if="selectedCountry"><svg class="w-5 h-auto rounded-sm shadow-sm" viewBox="0 0 36 24" fill="none" x-html="selectedCountry.svg"></svg></template>
                                                <span class="text-sm font-bold text-gray-600 dir-ltr dark:text-gray-300" x-text="selectedCountry?.dial_code"></span>
                                            </button>
            ...
            <div class="flex flex-wrap gap-4 justify-end items-center pt-4">
                <a href="{{ route('offices.unverified.index') }}"
                    class="px-7 py-3 min-w-[140px] text-sm font-bold text-center text-gray-700 bg-white rounded-xl border border-gray-300 shadow-sm transition-all dark:text-gray-300 dark:bg-boxdark-2 dark:border-boxdark hover:bg-surface hover:text-on-surface dark:hover:bg-boxdark">
                    إلغاء التعديل
                </a>
                <button type="submit" :disabled="isSubmitting"
                    class="flex gap-2 justify-center items-center px-7 py-3 min-w-[200px] text-sm font-bold text-white rounded-xl shadow-md transition-all bg-primary hover:bg-primary-hover hover:shadow-lg hover:shadow-primary/30 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed">
