...
@section('content')
    <div class="pb-28 min-h-screen bg-slate-50/50 font-body" dir="rtl" x-data="{
        isSubmitting: false,
        activeItem: 0,
        branches: @js(old('branches', [['name' => '', 'city' => '', 'phone' => '', 'address' => '']])),
        ...
        removeBranch(index) {
            if (this.branches.length > 1) {
                ...
                if (this.activeItem === index) this.activeItem = Math.max(0, index - 1);
                else if (this.activeItem > index) this.activeItem--;
            }
        }
        ...
        <div class="sticky top-0 z-[100] w-full bg-white/90 backdrop-blur-md border-b border-slate-100 shadow-sm">
            <div class="flex justify-between items-center px-4 mx-auto max-w-7xl h-20 sm:px-6 lg:px-8">
                ...
                <div class="flex gap-4 items-center">
                    ...
                    <div>
                        <h1 class="text-lg font-black tracking-tight text-slate-800 md:text-xl font-headline">إضافة مكتب جديد</h1>
                        <p class="text-[11px] font-bold text-slate-500 mt-0.5">تسجيل البيانات الأساسية والفروع</p>
                    </div>
                </div>
                ...
                <div class="flex gap-4 items-center">
                    ...
                    <button type="button" @click="document.getElementById('officeForm').requestSubmit()"
                        ...
                        class="flex gap-2 justify-center items-center px-6 h-12 text-sm font-bold rounded-2xl shadow-lg transition-all active:scale-95 shadow-primary/20"
        ...
        <div class="px-4 pt-8 mx-auto max-w-7xl sm:px-6 lg:px-8">
            <form id="officeForm" action="{{ route('offices.store') }}" method="POST"
                ...
                <div class="space-y-8">
                    ...
                    {{-- 1. كارد البيانات الأساسية (متوسط العرض) --}}
                    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden max-w-3xl mx-auto w-full">
                        <div class="p-8">
                            <div class="flex flex-col items-center mb-8">
                                ...
                                <h3 class="text-lg font-black text-slate-800 font-headline">البيانات الأساسية للمكتب</h3>
                            </div>
                            <div class="relative mx-auto max-w-md group">
                                <label class="block mb-2 text-[11px] font-bold text-slate-500 mr-2">اسم المكتب <span class="text-rose-500">*</span></label>
                                <input type="text" name="name" value="{{ old('name') }}" required placeholder="مثال: مكتب الإنجاز السريع"
                                ...
                                <span class="absolute right-4 bottom-4 transition-colors text-slate-400 material-symbols-outlined group-focus-within:text-primary">domain</span>
                            </div>
                        </div>
                    </div>

                    {{-- 2. قسم الفروع (شبكة عمودين) --}}
                    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-visible">
                        <div class="flex justify-between items-center px-8 py-6 mb-6 border-b border-slate-50">
                            <div class="flex gap-3 items-center">
                                ...
                                <h3 class="text-base font-black text-slate-800 font-headline">الفروع التابعة للمكتب</h3>
                            </div>
                            <button type="button" @click="addBranch()"
                                class="flex gap-2 items-center px-5 h-11 text-xs font-bold text-white bg-emerald-500 rounded-xl shadow-md transition-all hover:bg-emerald-600 active:scale-95 shadow-emerald-500/20">
                                <span class="material-symbols-outlined text-[18px]">add</span> إضافة فرع جديد
                            </button>
                        </div>
                            ...
                            <template x-for="(branch, index) in branches" :key="index">
                                <div :class="activeItem === index ? 'ring-2 ring-primary/40 shadow-xl z-30 scale-[1.02]' : 'border border-slate-100 z-10 opacity-90'"
                                    ...
                                    <div @click="activeItem = activeItem === index ? null : index"
                                        ...
                                        <div class="flex gap-4 items-center">
                                            <div class="flex justify-center items-center w-10 h-10 text-xs font-black rounded-xl shadow-inner transition-colors"
                                                :class="activeItem === index ? 'bg-primary text-white' : 'bg-slate-100 text-slate-400'" x-text="index + 1"></div>
                                            <div class="flex flex-col">
                                                <h4 class="text-sm font-black transition-colors" :class="activeItem === index ? 'text-primary' : 'text-slate-700'"
                                                    x-text="branch.name ? branch.name : 'فرع جديد'"></h4>
                                                <p class="text-[10px] font-bold text-slate-400" x-text="branch.city || 'يرجى تحديد المدينة'"></p>
                                            </div>
                                        </div>
                                        <div class="flex gap-2 items-center">
                                            ...
                                            <span class="transition-transform duration-300 material-symbols-outlined text-slate-300" :class="activeItem === index ? 'rotate-180 text-primary' : ''">expand_more</span>
                                        </div>
                                    </div>

                                    {{-- محتوى الفرع (الحقول مقسمة عمودين) --}}
                                    <div x-show="activeItem === index" x-collapse>
                                        <div class="p-6 pt-2 space-y-5 border-t border-slate-50">
                                            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                                                {{-- اسم الفرع --}}
                                                <div class="space-y-1.5">
                                                    <label class="text-[10px] font-black text-slate-400 mr-2 uppercase">اسم الفرع <span class="text-rose-500">*</span></label>
                                                    <input type="text" x-model="branch.name" :name="`branches[${index}][name]`" required placeholder="مثلاً: فرع الحصبة"
                                                        class="px-4 w-full h-12 text-xs font-bold rounded-2xl border transition-all outline-none border-slate-200 focus:border-primary focus:ring-4 focus:ring-primary/10 bg-slate-50/50">
                                                </div>

                                                {{-- المدينة --}}
                                                <div class="space-y-1.5">
                                                    <label class="text-[10px] font-black text-slate-400 mr-2 uppercase">المدينة <span class="text-rose-500">*</span></label>
                                                    <input type="text" x-model="branch.city" :name="`branches[${index}][city]`" required placeholder="صنعاء، عدن..."
                                                        class="px-4 w-full h-12 text-xs font-bold rounded-2xl border transition-all outline-none border-slate-200 focus:border-primary focus:ring-4 focus:ring-primary/10 bg-slate-50/50">
                                                </div>

                                                {{-- هاتف الفرع (Smart Combobox) --}}
                                                <div class="space-y-1.5">
                                                    <label class="text-[10px] font-black text-slate-400 mr-2 uppercase">رقم الهاتف</label>
                                                    <div x-data="phoneComponent()" x-init="initPhone(branch.phone)" x-effect="branch.phone = (selectedCountry?.dial_code.replace('+', '') || '') + localPhoneNumber"
                                                        ...
                                                        <div class="flex overflow-hidden items-center w-full h-12 rounded-2xl border border-slate-200 focus-within:border-primary focus-within:ring-4 focus-within:ring-primary/10 bg-slate-50/50">
                                                            ...
                                                            <button type="button" @click="open = !open" class="flex gap-1 items-center px-3 h-full bg-white border-r transition-colors border-slate-200 hover:bg-slate-50">
                                                                <template x-if="selectedCountry">
                                                                    <div class="flex overflow-hidden items-center w-6 h-4 rounded-sm shadow-sm" x-html="selectedCountry.svg"></div>
                                                                </template>
                                                                <span class="material-symbols-outlined text-[16px] text-slate-400">expand_more</span>
                                                            </button>
                                                        </div>
                                                        {{-- قائمة الدول --}}
                                                        <div x-show="open" x-transition x-cloak class="absolute left-0 right-0 top-full mt-2 max-h-48 overflow-y-auto bg-white border border-slate-100 shadow-2xl rounded-2xl z-[100] p-1">
                                                            <template x-for="country in filteredCountries" :key="country.code">
                                                                <button type="button" @click="selectedCountry = country; open = false;" 
                                                                    ...
                                                                    <div class="flex gap-3 items-center">
                                                                        <div class="flex overflow-hidden items-center w-5 h-3.5 rounded-sm" x-html="country.svg"></div>
                                                                        <span class="text-xs font-bold text-slate-700 group-hover:text-primary" x-text="country.name"></span>
                                                                    </div>
                                                                    <span class="font-mono text-[10px] text-slate-400 group-hover:text-primary/70" x-text="country.dial_code"></span>
                                                                </button>
                                                            </template>
                                                        </div>
                                                    </div>
                                                ...
                                                {{-- العنوان التفصيلي --}}
                                                <div class="space-y-1.5">
                                                    <label class="text-[10px] font-black text-slate-400 mr-2 uppercase">العنوان التفصيلي</label>
                                                    <input type="text" x-model="branch.address" :name="`branches[${index}][address]`" placeholder="الشارع، المعلم..."
                                                        class="px-4 w-full h-12 text-xs font-bold rounded-2xl border transition-all outline-none border-slate-200 focus:border-primary focus:ring-4 focus:ring-primary/10 bg-slate-50/50">
                                                </div>
                                            </div>
                                        </div>
    ...
    <script>
        function phoneComponent() {
            return {
                ...
                get filteredCountries() { return this.countries; }
            }
        }
    </script>
@endsection