...
@section('content')
    <div class="max-w-xl mx-auto space-y-8 font-outfit p-6">
        ...
        <div
            ...
            <div
                ...
                <div>
                    <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">الفرع المرسل (فرعك الحالي)</span>
                    <div class="text-base font-bold text-gray-800 dark:text-white mt-1">
                        {{ auth()->user()->branch->name ?? 'غير محدد' }}</div>
                </div>
                <div
            ...
            <div class="p-8">
                <form action="{{ route('finance.settlements.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        <label class="text-sm font-bold text-gray-700 dark:text-gray-300">الفرع المستلم (الداين)</label>
                        <div class="relative">
                            <select name="receiver_branch_code" required
                                ...
                                <option disabled selected value="">اختر الفرع المستحق للدفع...</option>
                                @foreach ($branchesOwed as $otherBranchId => $row)
                                    <option value="{{ $otherBranchId }}" data-amount="{{ abs($row['net']) }}">
                                        {{ $row['branch']->name ?? $otherBranchId }} — مستحق له:
                                        {{ number_format(abs($row['net']), 2) }} ر.ي
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="space-y-2">
                        <label class="text-sm font-bold text-gray-700 dark:text-gray-300 flex justify-between">
                            ...
                            <span id="max-amount-label" class="text-xs text-brand-500 hidden">الحد الأقصى المتاح</span>
                        </label>
                    ...
                    <!-- Description Input -->
                    <div class="space-y-2">
                        <label class="text-sm font-bold text-gray-700 dark:text-gray-300">ملاحظات (اختياري)</label>
                        <textarea name="description" rows="3"
                            ...
                            placeholder="مثال: تسليم نقد يد بيد..."></textarea>
                    </div>
                    ...
                    <p class="text-xs text-center text-gray-400">
                        سيتم تسجيل هذه العملية كصادر نقد من فرعك ووارد نقد للفرع المستلم.
                    </p>
                </form>
            </div>
        </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // Pass PHP array to JS safely if needed, but here we can stick to data-attributes for simpler logic
            // Or revert to the user's logic if they prefer the full object. 
            // Let's use the data-amount attribute approach for cleaner separation, 
            // assuming the PHP loop above sets it correctly. 
            // However, the user's script used the `owed` object. Let's stick to their logic but adapt it.
            const owed = @json($branchesOwed);

            selectBranch.addEventListener("change", function() {
                const id = this.value;

                if (owed[id] && owed[id].net) {
                    const net = Math.abs(owed[id].net); // Logic from user script
                    ...
                    maxLabel.textContent = "المستحق: " + net.toFixed(2) + " ر.ي";
                    maxLabel.classList.remove('hidden');

                    // Highlight the input temporarily to show it changed
                    amountInput.classList.add('ring-2', 'ring-brand-500/50');
                    setTimeout(() => amountInput.classList.remove('ring-2', 'ring-brand-500/50'), 500);
                }
            });
        });
    </script>
