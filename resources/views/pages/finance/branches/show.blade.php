                ...
                <p class="mt-2 text-lg font-semibold">
                    ...
                    @if ($net > 0)
                        <span class="text-success-600 dark:text-success-400">
                            لنا {{ number_format($net, 2) }} ر.ي
                        </span>
                    @elseif ($net < 0)
                        <span class="text-error-600 dark:text-error-400">
                            علينا {{ number_format(abs($net), 2) }} ر.ي
                        </span>
                    @else
                        ...
                        @forelse ($byCounterparty as $row)
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if ($net > 0)
                                        <span
                                            class="rounded-full bg-success-50 px-2 py-0.5 text-xs font-medium text-success-600 dark:bg-success-500/15 dark:text-success-400">
                                            عليه {{ number_format($net, 2) }} ر.ي
                                        </span>
                                    @elseif ($net < 0)
                                        <span
                                            class="rounded-full bg-error-50 px-2 py-0.5 text-xs font-medium text-error-600 dark:bg-error-500/15 dark:text-error-400">
                                            له {{ number_format(abs($net), 2) }} ر.ي
                                        </span>
                                    @else
                                        <span
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            ...
            <div class="max-w-full overflow-x-auto">
                <table class="min-w-full">
                    <thead>
                        <tr class="border-y border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/40">
                            <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400">التاريخ
                            </th>
                            ...
                            <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400">المبلغ
                            </th>
                        ...
                        @forelse ($transactions as $t)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-sm">
                                    ...
                                    @else
                                        <span
                                            class="rounded-full bg-amber-50 px-2 py-0.5 text-xs font-medium text-amber-600 dark:bg-amber-500/15 dark:text-amber-400">
                                            تسوية
                                        </span>
                                    @endif
                                </td>
