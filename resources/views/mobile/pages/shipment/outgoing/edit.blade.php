...
@section('title', 'تعديل الطرد - ' . $shipment->id)
...
<div x-data="{ paymentMethod: '{{ old('payment_method', $shipment->payment_method) }}' }"
    ...
    <div class="flex gap-4 items-center mb-2">
        ...
        <div>
            ...
            <p class="mt-1 text-xs font-bold text-primary">{{ $shipment->id }}</p>
        </div>
    </div>
    ...
    <form action="{{ route('shipment.outgoing.update', $shipment->id) }}" method="POST" class="space-y-6"
        ...
        {{-- ================= الغلاف الرئيسي (لربط الحسابات بين الطرد والمالية في وضع التعديل) ================= --}}
<div class="flex flex-col gap-4"
    x-data="{
        packageFee: {{ old('package_fee', $shipment->package_fee ?? 0) }},
        packageCommissionRate: {{ old('package_commission_rate', $shipment->package_commission_rate ?? 0) }},
        honeyFee: {{ old('honey_fee', $shipment->honey_fee ?? 0) }},
        honeyCommissionRate: {{ old('honey_commission_rate', $shipment->honey_commission_rate ?? 0) }},
    ...
    <div class="bg-white p-5 rounded-[1.75rem] border border-slate-50 shadow-[0_8px_30px_rgb(0,0,0,0.03)]">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div class="flex flex-col gap-2 col-span-2" x-data="{
                ...
                <div class="relative z-[40]">
                    <input type="text" name="package_type" x-model="packageType" @focus="isOpen = true"
                        ...
                        class="px-4 w-full h-12 text-sm font-bold rounded-xl border-none ring-1 outline-none bg-slate-50 ring-slate-100 focus:bg-white focus:ring-2 focus:ring-primary/20 text-slate-700">

                    <button type="button" @click="isOpen = !isOpen" tabindex="-1"
                        class="flex absolute inset-y-0 right-0 items-center px-3 h-full rounded-l-xl transition-colors text-slate-400 hover:text-primary">
            ...
            <div class="flex flex-col gap-2 col-span-2">
                ...
                <input type="number" step="0.1" name="weight" placeholder="مثال: 2.5"
                    value="{{ old('weight', $shipment->weight) }}" min="0"
                    class="px-4 w-full h-12 text-sm text-left rounded-xl border-none ring-1 outline-none bg-slate-50 ring-slate-100 focus:bg-white focus:ring-2 focus:ring-primary/20 text-slate-700"
                    dir="ltr">
            </div>

            {{-- 3. تسعيرة الطرد وعمولته --}}
            <div class="flex flex-col gap-2 col-span-1 mt-1">
                <label class="text-[11px] font-bold text-slate-400">أجرة الطرد (ريال) <span class="text-rose-500">*</span></label>
                <input type="number" name="package_fee" x-model.number="packageFee" placeholder="0" min="0" dir="ltr" required
                    class="px-4 w-full h-12 text-sm font-black text-left text-blue-600 rounded-xl border-none ring-1 outline-none bg-slate-50 ring-slate-100 focus:bg-white focus:ring-2 focus:ring-blue-500/20">
            </div>

            <div class="flex flex-col gap-2 col-span-1 mt-1">
                <label class="text-[11px] font-bold text-slate-400">نسبة العمولة (%)</label>
                <div class="relative">
                    <input type="number" name="package_commission_rate" x-model.number="packageCommissionRate" placeholder="0" min="0" max="100" dir="ltr"
                        class="px-4 w-full h-12 text-sm font-bold text-left rounded-xl border-none ring-1 outline-none bg-slate-50 ring-slate-100 focus:bg-white focus:ring-2 focus:ring-primary/20 text-slate-700">
                    ...
                    <div class="absolute -bottom-5 left-1 flex gap-1 items-center text-[9px] font-bold text-emerald-600">
                        ...
                        عمولتك: <span x-text="packageCommissionCalc"></span>
                    </div>
                </div>
            </div>
        ...
        {{-- ================= منطقة بيانات العسل ================= --}}
        <div x-data="{ showHoneyFields: {{ (old('no_gallons_honey', $shipment->no_gallons_honey) > 0 || old('no_honey_jars', $shipment->no_honey_jars) > 0 || old('honey_fee', $shipment->honey_fee) > 0) ? 'true' : 'false' }} }" class="mt-8">
            ...
            <div x-show="showHoneyFields" x-cloak x-collapse>
                <div class="grid grid-cols-2 gap-4 p-4 mt-2 rounded-xl border bg-amber-50/50 border-amber-100/50">
                    ...
                    <div>
                        ...
                        <input type="number" name="no_gallons_honey" value="{{ old('no_gallons_honey', $shipment->no_gallons_honey) }}" placeholder="العدد" min="0"
                    ...
                    <div>
                        ...
                        <input type="number" name="no_honey_jars" value="{{ old('no_honey_jars', $shipment->no_honey_jars) }}" placeholder="العدد" min="0"
                            class="px-3 w-full h-11 text-sm bg-white rounded-lg border border-amber-100 transition-all outline-none focus:border-amber-400">
                    </div>
                    ...
                    <div class="mt-1 relative">
                        ...
                        <div class="absolute -bottom-5 left-1 flex gap-1 items-center text-[9px] font-bold text-emerald-600">
                            <span class="material-symbols-outlined text-[10px]">payments</span>
                            عمولتك: <span x-text="honeyCommissionCalc"></span>
                        </div>
                    </div>
        ...
        {{-- 4. الملاحظات --}}
        <div class="flex flex-col gap-2 pt-2 mt-6">
            ...
            <textarea name="notes" rows="2" placeholder="اكتب أي ملاحظات هنا..."
                class="p-4 w-full text-sm rounded-2xl border-none ring-1 outline-none resize-none bg-slate-50 ring-slate-100 focus:bg-white focus:ring-2 focus:ring-primary/20 text-slate-700">{{ old('notes', $shipment->notes) }}</textarea>
        </div>
    </div>
    ...
    <div class="bg-white p-5 rounded-[1.75rem] border border-slate-50 shadow-[0_8px_30px_rgb(0,0,0,0.03)]">
        ...
        <div class="space-y-4">
            <div class="flex flex-col gap-2">
                ...
                <select name="payment_method" x-model="paymentMethod"
                    ...
                    @hasservice('Payment_Prepaid')
                        <option value="prepaid" {{ old('payment_method', $shipment->payment_method) === 'prepaid' ? 'selected' : '' }}>مدفوع مقدماً</option>
                    @endhasservice
                    @hasservice('Payment_COD')
                        <option value="cod" {{ old('payment_method', $shipment->payment_method) === 'cod' ? 'selected' : '' }}>الدفع عند الاستلام (على حساب المستلم)</option>
                    @endhasservice
                    @hasservice('Payment_Partial')
                        <option value="partial_payment" {{ old('payment_method', $shipment->payment_method) === 'partial_payment' ? 'selected' : '' }}>دفع جزئي</option>
                    @endhasservice
                    @hasservice('Payment_Credit')
                        <option value="customer_credit" {{ old('payment_method', $shipment->payment_method) === 'customer_credit' ? 'selected' : '' }}>آجل (على حساب المرسل)</option>
                    @endhasservice
                </select>
            </div>
            ...
            <div class="flex flex-col gap-2">
                ...
                {{-- 💡 حقل مقفل يحسب المجموع تلقائياً --}}
            ...
            <div x-show="paymentMethod === 'partial_payment'" x-collapse class="flex flex-col gap-2">
                ...
                <input type="number" name="partial_amount" placeholder="0.00" min="0" :max="totalAmount"
