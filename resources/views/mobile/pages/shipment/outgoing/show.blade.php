...
@section('content')
    ...
    <div class="flex relative flex-col gap-5 px-4 pt-6 pb-8 min-h-screen bg-slate-50/50" x-data="{ showPaymentModal: false, showRefundModal: false }">
        ...
        <div class="flex gap-3 items-center mt-1 relative z-[60]">
            ...
            @if (!empty($availableStatuses))
                <div class="flex-[2] relative" x-data="{ openStatusMenu: false }">
                    ...
                    <form action="{{ route('shipment.updateStatus', $shipment->id) }}" method="POST" x-ref="statusForm" id="updateStatusFormMobile">
                        @csrf
        ...
        {{-- ================= بطاقة المسار (من -> إلى) ================= --}}
        <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)] relative overflow-hidden mt-2">
            <div class="absolute -top-10 -right-10 w-40 h-40 rounded-full blur-3xl pointer-events-none bg-primary/5"></div>
            <div class="absolute -bottom-10 -left-10 w-40 h-40 rounded-full blur-3xl pointer-events-none bg-emerald-500/5"></div>

            <div class="flex relative z-10 justify-between items-center mb-8">
                <div class="flex gap-3 items-center">
                    <div class="flex justify-center items-center w-10 h-10 bg-gradient-to-br rounded-xl shadow-inner from-primary/20 to-primary/5 text-primary">
            ...
            <div class="relative pr-6 pl-2 space-y-8">
                ...
                <div class="relative z-10">
                    ...
                    <div class="p-3.5 rounded-2xl border transition-colors bg-slate-50/50 border-slate-100/50 hover:bg-slate-50">
                        <span class="text-[10px] font-black text-slate-400 mb-1 block">المرسل • {{ $shipment->senderBranch?->name ?? 'مكتب خارجي' }}</span>
                        <div class="flex justify-between items-center">
                            <div>
                                <span class="block text-sm font-black text-slate-800">{{ $shipment->senderCustomer?->name ?? 'عميل نقدي' }}</span>
                ...
                <div class="relative z-10">
                    ...
                    <div class="bg-primary/[0.02] p-3.5 rounded-2xl border border-primary/10 hover:bg-primary/5 transition-colors">
                        ...
                        <div class="flex justify-between items-center">
                            <div>
                                <span class="block text-sm font-black text-primary">{{ $shipment->receiverCustomer?->name ?? 'عميل نقدي' }}</span>
                ...
                <div class="flex relative z-10 flex-col gap-3">
                    <div class="flex justify-between items-center p-4 rounded-2xl border border-blue-50 shadow-sm backdrop-blur-sm bg-white/80">
                        <div class="flex gap-3 items-center">
                            ...
                            <div>
                                <p class="text-[9px] font-black text-slate-400 mb-0.5 uppercase tracking-wider">السائق المسؤول</p>
                                <p class="text-xs font-black text-slate-800">{{ $shipment->package->driver->name ?? 'غير محدد' }}</p>
                        ...
                        @if ($shipment->package->driver && $shipment->package->driver->phone)
                            <div class="flex gap-1.5 items-center">
                                <a href="https://wa.me/{{ ltrim($shipment->package->driver->phone, '+') }}?text={{ urlencode($driverMsg) }}" target="_blank"
                    ...
                    <div class="flex justify-between items-center p-4 bg-blue-600 rounded-2xl shadow-lg shadow-blue-600/20">
                        <div>
                            ...
                            <p class="font-mono text-sm font-black tracking-widest text-white">{{ $shipment->package->tracking_number }}</p>
                        </div>
        ...
        <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)]">
            <div class="flex gap-3 items-center mb-5">
                <div class="flex justify-center items-center w-10 h-10 bg-gradient-to-br rounded-xl shadow-inner from-slate-100 to-slate-50 text-slate-600">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div class="flex flex-col gap-1.5 p-4 rounded-2xl border bg-slate-50/80 border-slate-100">
                    ...
                    <span class="flex gap-1 items-center text-sm font-black text-slate-800">{{ $shipment->package_type }}</span>
                </div>
                @if ($shipment->weight > 0)
                    <div class="flex flex-col gap-1.5 p-4 rounded-2xl border bg-slate-50/80 border-slate-100">
                        <span class="text-[10px] font-black text-slate-400 uppercase tracking-wider">الوزن الفعلي</span>
                        <span class="text-sm font-black text-slate-800">{{ $shipment->weight }} <span class="text-xs font-bold text-slate-500">كجم</span> ⚖️</span>
                    </div>
                @endif
                @if ($shipment->no_gallons_honey > 0 || $shipment->no_honey_jars > 0)
                    <div class="flex overflow-hidden relative col-span-2 justify-around items-center p-4 bg-gradient-to-r from-amber-50 to-orange-50 rounded-2xl border border-amber-100/50">
                        ...
                        @if ($shipment->no_gallons_honey > 0)
                            <div class="flex relative z-10 flex-col gap-1 items-center">
                                ...
                                <span class="flex gap-1 items-center text-xl font-black text-amber-700">{{ $shipment->no_gallons_honey }} <span class="text-sm">🍯</span></span>
                            </div>
                        @endif
                        @if ($shipment->no_gallons_honey > 0 && $shipment->no_honey_jars > 0)
                        ...
                        @if ($shipment->no_honey_jars > 0)
                            <div class="flex relative z-10 flex-col gap-1 items-center">
                                ...
                                <span class="flex gap-1 items-center text-xl font-black text-amber-700">{{ $shipment->no_honey_jars }} <span class="text-sm">🏺</span></span>
                            </div>
                        @endif
                    </div>
                @endif
        ...
        <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)]">
            <div class="flex justify-between items-center mb-6">
                <div class="flex gap-3 items-center">
                    <div class="flex justify-center items-center w-10 h-10 text-emerald-600 bg-gradient-to-br from-emerald-100 to-emerald-50 rounded-xl shadow-inner">
            ...
            <div class="p-4 space-y-4 rounded-2xl border bg-slate-50/50 border-slate-100">
                ...
                @if ($shipment->payment_method == 'partial_payment')
                    ...
                    <div class="flex justify-between items-end">
                        ...
                        <span class="text-2xl font-black text-rose-600">{{ number_format($remainingAmount, 0) }} <span class="text-xs font-bold text-rose-400">ريال</span></span>
                    </div>
                @elseif($shipment->payment_method == 'cod')
                    <div class="my-2 border-t border-dashed border-slate-200"></div>
                    <div class="flex justify-between items-end">
        ...
        @if(!$shipment->is_returned && !in_array($shipment->payment_method, ['prepaid', 'customer_credit']))
            <template x-teleport="body">
                <div x-show="showPaymentModal" x-cloak class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                    <div x-show="showPaymentModal" x-transition.opacity duration.300ms @click="showPaymentModal = false" class="absolute inset-0 backdrop-blur-md bg-slate-900/60"></div>
                    ...
                    <div x-show="showPaymentModal" x-transition:enter="transition ease-out duration-300"
                        ...
                        <div class="flex justify-center items-center mx-auto mb-4 w-20 h-20 text-emerald-500 bg-emerald-50 rounded-full animate-bounce">
                        ...
                        <p class="mb-6 text-xs font-bold leading-relaxed text-slate-500">
                            هذا الطرد غير مدفوع مسبقاً بالكامل. الرجاء استلام المبلغ التالي من العميل قبل تأكيد عملية التسليم.
                        </p>
                        ...
                        <div class="flex gap-2">
                            ...
                            <button type="button" @click="document.getElementById('updateStatusInputMobile').value = 'delivered'; document.getElementById('updateStatusFormMobile').submit();"
        ...
        @if($shipment->is_returned && $refundAmount > 0)
            <template x-teleport="body">
                <div x-show="showRefundModal" x-cloak class="fixed inset-0 z-[9999] flex items-center justify-center p-4">
                    <div x-show="showRefundModal" x-transition.opacity duration.300ms @click="showRefundModal = false" class="absolute inset-0 backdrop-blur-md bg-slate-900/60"></div>
                    ...
                    <div x-show="showRefundModal" x-transition:enter="transition ease-out duration-300"
                        ...
                        <div class="flex justify-center items-center mx-auto mb-4 w-20 h-20 text-amber-500 bg-amber-50 rounded-full animate-bounce">
                        ...
                        <p class="mb-6 text-xs font-bold leading-relaxed text-slate-500">
                            هذا الطرد مرتجع للتاجر وقد تم دفع رسومه مسبقاً. الرجاء إرجاع المبلغ التالي للعميل يدوياً قبل إغلاق دورة الطرد.
                        </p>
                        ...
                        <div class="flex gap-2">
                            ...
                            <button type="button" @click="document.getElementById('updateStatusInputMobile').value = 'returned'; document.getElementById('updateStatusFormMobile').submit();"
