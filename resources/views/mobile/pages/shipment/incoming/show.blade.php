            ...
            @else
                ...
                <div class="flex-[2] relative" x-data="{ openStatusMenu: false, showPaymentModal: false, isSubmitting: false }">
                    <form action="{{ route('shipment.updateStatus', $shipment->id) }}" method="POST" x-ref="statusForm"
                        ...
                        {{-- 💡 السحر هنا: النافذة تظهر فقط إذا كان هناك مبلغ متبقي للتحصيل --}}
                        @if(!$shipment->is_returned && $shipment->amount_to_collect_from_receiver > 0)
                ...
                <div class="flex items-center justify-between p-4 bg-emerald-50/50 border border-emerald-100 rounded-2xl">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-black text-emerald-700">خالص / لا يوجد تحصيل</p>
                        </div>
                    </div>
                    <span class="text-[10px] font-bold text-emerald-600 bg-emerald-100 px-2 py-1 rounded-lg">جاهز للتسليم</span>
                </div>
            @else
