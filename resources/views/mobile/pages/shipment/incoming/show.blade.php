...
@section('content')
    <div class="flex relative flex-col gap-5 px-4 pt-6 pb-8 min-h-screen bg-slate-50/50" x-data="{ 
        ...
        {{-- ================= بطاقة العميل المستلم (مع أزرار التواصل) ================= --}}
        @php
        ...
        <div class="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)]">
            ...
            <div class="p-4 bg-slate-50/50 border border-slate-100 rounded-2xl">
                <p class="text-sm font-black text-slate-800">{{ $shipment->receiverCustomer->name ?? 'غير مسجل' }}</p>
                <p class="text-xs font-bold text-slate-500 mt-1 dir-ltr text-right">
                    {{ $shipment->receiverCustomer->phone ?? 'لا يوجد رقم' }}</p>
                ...
                @if($shipment->receiverCustomer && $shipment->receiverCustomer->phone)
                    <div class="flex gap-2 mt-4 pt-4 border-t border-slate-200/60">
                        <a href="https://wa.me/{{ ltrim($shipment->receiverCustomer->phone, '+') }}?text={{ urlencode($whatsappMsg) }}"
                            target="_blank"
                            class="flex-1 h-10 bg-[#25D366]/10 text-[#25D366] rounded-xl flex items-center justify-center gap-2 hover:bg-[#25D366]/20 transition-colors font-bold text-xs">
                        ...
                        <a href="tel:{{ $shipment->receiverCustomer->phone }}"
                            class="flex-1 h-10 bg-slate-800 text-white rounded-xl flex items-center justify-center gap-2 hover:bg-slate-700 transition-colors font-bold text-xs">
