    ...
    <div class="px-4 py-10 min-h-screen font-sans bg-slate-50" dir="rtl">
        ...
        <div class="flex flex-col gap-6">
            @foreach($packages as $package)
                @php
                    // 💡 التحقق من حالة الباقة بالنسبة للعميل
                ...
                <div class="bg-white rounded-2xl shadow-xl p-8 border 
                    @if($isActiveThis) border-blue-500 
                    @elseif($isPendingThis) border-green-500 
                    @else border-gray-100 @endif 
                    transition-all relative overflow-hidden">

                    @if($isActiveThis)
                        <div class="absolute top-0 left-0 bg-blue-500 text-white text-xs px-3 py-1 rounded-br-lg font-bold">
                    ...
                    @if($isActiveThis)
                        <button disabled class="w-full bg-gray-100 text-gray-500 border border-gray-300 font-bold py-4 rounded-xl cursor-not-allowed">
