...
@section('content')
    <div class="flex relative flex-col gap-5 px-4 pt-6 pb-8 min-h-screen bg-slate-50/50" x-data="{ 
        ...
        {{-- ================= أزرار الإجراءات (تأكيد الاستلام) ================= --}}
        <div class="flex gap-3 items-center mt-1">
            ...
            @php
                ...
                // بالنسبة للوارد، الإجراء الأهم هو تأكيد الاستلام عندما تكون في الطريق
                ...
                <div class="flex-[2] relative" x-data="{ openStatusMenu: false }">
                    <form action="{{ route('shipmentpackage.updateStatus', $package->id) }}" method="POST" x-ref="statusForm" @submit="isSubmitting = true">
                        ...
                        <button type="button" @click="openStatusMenu = !openStatusMenu" @click.outside="openStatusMenu = false"
                            class="w-full flex items-center justify-between px-4 h-12 bg-emerald-600 text-white rounded-2xl font-bold text-xs shadow-[0_8px_20px_rgba(5,150,105,0.2)] hover:bg-emerald-700 active:scale-95 transition-all">
                            <div class="flex gap-2 items-center">
                                <span class="material-symbols-outlined text-[18px]">download_done</span>
                                <span>إجراءات الاستلام</span>
                            </div>
                        ...
                        <div x-show="openStatusMenu" x-cloak x-transition:enter="transition ease-out duration-200"
                            ...
                            <div class="px-3 py-2 mb-1 border-b border-slate-50">
                                <span class="text-[9px] font-bold text-slate-400">بمجرد التأكيد ستصبح الطرود في عهدتك.</span>
                            </div>

                            @foreach($availableStatuses as $value => $data)
                                <button type="button" @click="$refs.statusInput.value = '{{ $value }}'; $refs.statusForm.submit()" :disabled="isSubmitting"
        ...
        <div
            ...
            <div class="flex relative z-10 justify-between items-center mb-8">
                <div class="flex gap-3 items-center">
                    ...
                    <h3 class="text-sm font-black text-slate-800 font-headline">مسار الإرسالية الواردة</h3>
                </div>
            ...
            <div class="relative z-10 pr-6 pl-2 space-y-8">
                ...
                {{-- 1. مصدر الانطلاق (فرع داخلي أو مكتب خارجي) --}}
                <div class="relative z-10">
                    ...
                    <div
                        ...
                        <span class="text-[10px] font-black text-slate-400 mb-1 block">مصدر الإرسالية</span>
                        <div class="flex justify-between items-center">
                            <div>
                                <span
                                    class="block text-sm font-black text-slate-800">{{ $package->sender_entity->name ?? 'غير معروف' }}</span>
                                <span class="block flex gap-1 items-center mt-0.5 text-[10px] font-bold text-slate-500">
                ...
                <div class="relative z-10">
                    ...
                    <div
                        ...
                        <span class="text-[10px] font-black text-emerald-600/70 mb-2 block">الوجهة (إلينا)</span>
