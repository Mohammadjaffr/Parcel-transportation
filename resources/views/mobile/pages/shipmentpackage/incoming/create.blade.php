...
<div class="pb-24 min-h-screen bg-surface dark:bg-boxdark-2 font-body lg:pb-12" dir="rtl">
    ...
    <div class="sticky top-0 z-40 border-b border-gray-100 shadow-sm backdrop-blur-md bg-white/90 dark:bg-boxdark/90 dark:border-boxdark-2">
        <div class="flex justify-between items-center px-4 py-4 mx-auto max-w-7xl md:px-6">
            <div class="flex gap-4 items-center">
                ...
                <div>
                    <h1 class="text-xl font-black md:text-2xl font-headline text-on-surface dark:text-white">استلام إرسالية</h1>
    ...
    <div class="p-4 mx-auto mt-4 max-w-7xl md:p-6">
        ...
        <form id="incomingPackageForm" action="{{ route('shipmentpackage.incoming.store') }}" method="POST" @submit="isSubmitting = true" 
            ...
            <div class="lg:col-span-5 flex flex-col gap-6 lg:sticky lg:top-[5.5rem] z-30">
                ...
                <div class="bg-white dark:bg-boxdark rounded-[2rem] border border-gray-100 dark:border-boxdark-2 shadow-sm overflow-hidden relative">
                    ...
                    <div class="relative z-10 p-5 space-y-6 md:p-6">
                        ...
                        <div>
                            ...
                            @error('tracking_number') <p class="mt-2 text-xs font-bold text-error">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div x-data="{
                            ...
                            <div>
                                ...
                                @error('office_id') <p class="mt-2 text-xs font-bold text-error">{{ $message }}</p> @enderror
                            </div>
                            ...
                            <div x-show="selectedOffice" x-transition:enter.duration.300ms x-cloak>
                                ...
                                @error('sender_office_branch_id') <p class="mt-2 text-xs font-bold text-error">{{ $message }}</p> @enderror
                            </div>
                        </div>
                        ...
                        <div class="pt-4 space-y-4 border-t border-gray-50 dark:border-boxdark-2">
                            ...
                            <div>
                                ...
                                <div class="flex overflow-visible relative items-center w-full h-12 rounded-xl border border-gray-200 transition-all bg-surface dark:bg-boxdark-2 dark:border-boxdark-2 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 focus-within:bg-white dark:focus-within:bg-boxdark"
                                    ...
                                    {{-- Input --}}
                                    <input type="tel" x-model="localPhoneNumber" @input="onPhoneInput()" @focus="driverOpen = true" @click.outside="driverOpen = false" placeholder="7XXXXXXXX" dir="ltr" autocomplete="off"
                                            ...
                                            <template x-for="country in filteredCountries" :key="country.code">
                                                <button type="button" @click="selectedCountry = country; countryOpen = false; countrySearch = ''" 
                                                    ...
                                                    <div class="flex gap-3 items-center">
                                                        <div class="w-5 h-auto rounded-[2px] shadow-sm overflow-hidden" x-html="country.svg"></div>
            ...
            <div class="lg:col-span-7 bg-white dark:bg-boxdark rounded-[2rem] border border-gray-100 dark:border-boxdark-2 shadow-sm overflow-visible relative">

                {{-- Header --}}
                <div class="flex justify-between items-center px-5 md:px-6 py-4 border-b border-gray-50 dark:border-boxdark-2 bg-surface dark:bg-boxdark rounded-t-[2rem]">
                    <div class="flex gap-3 items-center">
                        <div class="flex justify-center items-center w-10 h-10 text-orange-500 bg-orange-50 rounded-xl dark:bg-orange-500/10 shrink-0">
                ...
                <div class="p-4 md:p-6 space-y-4 bg-surface dark:bg-boxdark-2 rounded-b-[2rem]">
                    <template x-for="(item, index) in items" :key="index">
                        ...
                        <div :class="errorIndices.includes(index) ? 'border-error shadow-sm' : (activeItem === index ? 'border-primary shadow-md dark:shadow-none dark:border-primary/50' : 'border-gray-200 dark:border-boxdark hover:border-gray-300 dark:hover:border-gray-600')"
                            class="overflow-hidden relative bg-white rounded-2xl border transition-all duration-300 dark:bg-boxdark">
                            ...
                            <div @click="activeItem = activeItem === index ? null : index"
                                ...
                                <div class="flex gap-4 items-center">
                                    ...
                                    <div class="flex flex-col">
                                        <h4 class="text-sm font-black transition-colors font-headline" 
                                            :class="activeItem === index ? 'text-primary' : 'text-on-surface dark:text-white'"
                                            x-text="item.bond_number || 'طرد جديد (بدون رقم)'"></h4>
                                        <p class="text-[10px] font-bold mt-0.5"
                                           :class="activeItem === index ? 'text-gray-500 dark:text-bodydark' : 'text-gray-400 dark:text-gray-500'"
                                           x-text="item.receiver_name ? 'للمستلم: ' + item.receiver_name : 'يرجى إكمال بيانات الطرد...'"></p>
                                    </div>
                                </div>
                            ...
                            <div x-show="activeItem === index" x-collapse>
                                <div class="p-5 space-y-6 md:p-6">
                                    ...
                                    {{-- رقم السند --}}
                                    <div>
                                        <label class="block mb-2 text-xs font-bold text-gray-600 dark:text-gray-300">رقم البوليصة (السند) <span class="text-error">*</span></label>
                                    ...
                                    <div class="grid grid-cols-1 gap-6 xl:grid-cols-2">
                                        ...
                                        {{-- ================= بيانات المرسل (ذكي) ================= --}}
                                        <div class="p-4 space-y-4 rounded-2xl border border-gray-100 md:p-5 bg-surface dark:bg-boxdark-2 dark:border-boxdark" x-data="{
                                            ...
                                            <h4 class="pb-2 text-sm font-black text-gray-700 border-b border-gray-200 dark:text-gray-200 dark:border-boxdark font-headline">بيانات المرسل</h4>

                                            {{-- هاتف المرسل --}}
                                            <div class="relative">
                                                <label class="block mb-1.5 text-xs font-bold text-gray-500 dark:text-gray-400">رقم الهاتف</label>
                                                <input type="hidden" :name="`items[${index}][sender_phone]`" :value="item.sender_phone">

                                                <div class="flex overflow-visible items-center w-full h-12 bg-white rounded-xl border border-gray-200 transition-all dark:bg-boxdark dark:border-boxdark-2 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20">
                                                    ...
                                                    {{-- Country Btn --}}
                                                    <button type="button" @click="sdOpen = !sdOpen" class="flex gap-2 items-center px-3 h-full rounded-l-xl border-r border-gray-200 transition-colors bg-surface dark:bg-boxdark-2 dark:border-boxdark shrink-0 hover:bg-gray-100 dark:hover:bg-boxdark">
                                                        <template x-if="sdSelected">
                                                            <div class="flex gap-1.5 items-center">
                                                                <div class="w-5 h-auto rounded-[2px] shadow-sm overflow-hidden" x-html="sdSelected.svg"></div>
                                            ...
                                            {{-- اسم المرسل --}}
                                            <div>
                                                <label class="block mb-1.5 text-xs font-bold text-gray-500 dark:text-gray-400">الاسم (اختياري)</label>
                                                <input type="text" :name="`items[${index}][sender_name]`" x-model="item.sender_name" placeholder="اسم المرسل"
                                                    class="px-4 w-full h-12 text-sm font-bold bg-white rounded-xl border border-gray-200 transition-all outline-none dark:bg-boxdark dark:border-boxdark-2 focus:border-primary focus:ring-2 focus:ring-primary/20 text-on-surface dark:text-white">
                                            </div>
                                        </div>
                                        ...
                                        <div class="p-4 space-y-4 rounded-2xl border border-gray-100 md:p-5 bg-surface dark:bg-boxdark-2 dark:border-boxdark" x-data="{
                                            ...
                                            <div class="relative">
                                                ...
                                                <div class="flex overflow-visible items-center w-full h-12 bg-white rounded-xl border border-gray-200 transition-all dark:bg-boxdark dark:border-boxdark-2 focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-500/30">
                                                    ...
                                                    <button type="button" @click="rcOpen = !rcOpen" class="flex gap-2 items-center px-3 h-full rounded-l-xl border-r border-gray-200 transition-colors bg-surface dark:bg-boxdark-2 dark:border-boxdark shrink-0 hover:bg-gray-100 dark:hover:bg-boxdark">
                                                        <template x-if="rcSelected">
                                                            <div class="flex gap-1.5 items-center">
                                                                <div class="w-5 h-auto rounded-[2px] shadow-sm overflow-hidden" x-html="rcSelected.svg"></div>
                                                ...
                                                <div x-show="rcOpen && rcLocal.trim().length > 0" x-transition x-cloak
                                                    ...
                                                    <div x-show="rcFilteredCustomers.length === 0" class="flex gap-2 justify-center items-center px-4 py-3 m-1 text-xs font-bold text-gray-500 rounded-lg dark:text-bodydark bg-surface dark:bg-boxdark-2">
                                                        <span class="material-symbols-outlined text-[18px] text-emerald-500">person_add</span>
                                                        عميل جديد
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div class="grid grid-cols-1 gap-6 pt-6 border-t border-gray-100 md:grid-cols-12 dark:border-boxdark-2">
                                        ...
                                        <div class="space-y-4 md:col-span-7">
                                            <div class="grid grid-cols-2 gap-4">
                                                <div>
                                                    <label class="block mb-1.5 text-xs font-bold text-gray-600 dark:text-gray-300">نوع المحتوى <span class="text-error">*</span></label>
                                            ...
                                            <div>
                                                ...
                                                <div class="flex p-1 bg-gray-200/50 dark:bg-boxdark rounded-[14px] border border-gray-200 dark:border-boxdark-2">
                                                    <label class="flex-1 cursor-pointer">
                                                        ...
                                                        <div class="flex justify-center items-center w-full h-10 text-xs font-bold text-gray-500 rounded-xl transition-all dark:text-gray-400 peer-checked:bg-white dark:peer-checked:bg-boxdark-2 peer-checked:text-primary dark:peer-checked:text-white peer-checked:shadow-sm">
                                                            عند الاستلام
                                                        </div>
                                                    </label>
                                                    <label class="flex-1 cursor-pointer">
                                                        ...
                                                        <div class="flex justify-center items-center w-full h-10 text-xs font-bold text-gray-500 rounded-xl transition-all dark:text-gray-400 peer-checked:bg-white dark:peer-checked:bg-boxdark-2 peer-checked:text-emerald-500 dark:peer-checked:text-emerald-400 peer-checked:shadow-sm">
                                                            مدفوع
                                                        </div>
                                                    </label>
                                                </div>
                    ...
                    <div x-show="items.length === 0" class="flex flex-col justify-center items-center py-16 bg-surface dark:bg-boxdark-2 rounded-[2rem] border-2 border-gray-200 dark:border-boxdark border-dashed">
            ...
            <div class="md:hidden fixed bottom-6 left-4 right-4 p-4 bg-white/95 dark:bg-boxdark-2/95 backdrop-blur-md border border-gray-100 dark:border-boxdark rounded-[2rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] dark:shadow-black/50 z-40">
