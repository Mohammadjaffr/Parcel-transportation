    ...
    <div class="flex flex-col gap-5 px-4 pb-24 pt-4 min-h-screen bg-slate-50/50">
        ...
        <form action="{{ route('shipmentpackage.incoming.store') }}" method="POST" class="space-y-5" dir="rtl" @submit="isSubmitting = true" 
            ...
            <div class="bg-white rounded-[2rem] border border-slate-100 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)] overflow-hidden">
                <div class="px-5 py-4 border-b border-slate-50 bg-slate-50/50 flex items-center gap-3">
                ...
                <div class="p-5 space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative group">
                            <input type="text" name="tracking_number" value="{{ old('tracking_number') }}" required
                                ...
                                class="pl-4 pr-11 w-full h-12 text-sm font-bold bg-slate-50/50 rounded-xl border border-slate-200 outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 focus:bg-white transition-all text-slate-700">
                            <div class="absolute inset-y-0 right-0 flex items-center pr-3.5 text-slate-400 group-focus-within:text-primary transition-colors pointer-events-none">
                    ...
                    <div x-data="{
                        ...
                        <div>
                            ...
                            <select name="office_id" x-model="selectedOffice" @change="selectedBranch = ''" required
                                class="px-4 w-full h-12 text-sm font-bold bg-slate-50/50 rounded-xl border border-slate-200 outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 focus:bg-white transition-all text-slate-700 appearance-none">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select name="sender_office_branch_id" x-model="selectedBranch" required
                                    ...
                                    class="px-4 w-full h-12 text-sm font-bold rounded-xl border outline-none transition-all appearance-none">
                                ...
                                <div x-show="!selectedOffice" class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-slate-400">
                    ...
                    <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-4">
                        ...
                        <div>
                            ...
                            <div class="flex overflow-visible items-center w-full h-12 bg-white rounded-xl border border-slate-200 transition-all focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 relative"
                                ...
                                <input type="tel" x-model="localPhoneNumber" @input="onPhoneInput()"
                                    ...
                                    class="flex-grow min-w-0 px-3 sm:px-4 h-full text-sm font-bold tracking-wider text-left bg-transparent border-none outline-none focus:ring-0 text-slate-700">
                                ...
                                <button type="button" @click="countryOpen = !countryOpen"
                                    class="flex gap-1.5 items-center px-2 sm:px-3 h-full bg-slate-50 rounded-l-xl border-r border-slate-200 transition-colors hover:bg-slate-100 shrink-0">
                                ...
                                <div x-show="countryOpen" @click.outside="countryOpen = false" x-transition x-cloak
                                    ...
                                    <div class="p-2 border-b border-slate-50">
                                        <input type="text" x-model="countrySearch" placeholder="ابحث عن الدولة..."
                                            class="px-3 py-2 w-full text-xs font-bold bg-slate-50 rounded-xl border border-slate-200 outline-none focus:border-primary text-slate-700">
                                    </div>
                                    <div class="overflow-y-auto max-h-40 custom-scrollbar p-1">
                                        <template x-for="country in filteredCountries" :key="country.code">
                                            <button type="button" @click="selectedCountry = country; countryOpen = false; countrySearch = ''" 
                                                class="flex justify-between items-center px-3 py-2.5 w-full text-sm text-left transition-colors hover:bg-primary/5 rounded-lg">
                                                <div class="flex gap-3 items-center">
                                                    <img :src="`https://flagcdn.com/w20/${country.code.toLowerCase()}.png`" class="w-5 h-auto rounded-[2px] shadow-sm">
                                                    <span class="font-bold text-slate-700 text-xs" x-text="country.name"></span>
                                                </div>
                                ...
                                <div x-show="driverOpen && localPhoneNumber.trim().length > 0 && !driver_id" x-transition x-cloak @click.outside="driverOpen = false"
                                    ...
                                    <template x-for="driver in filteredDrivers" :key="driver.id">
                                        <button type="button" @click="selectDriver(driver)"
                                            class="flex justify-between items-center px-4 py-3 w-full text-right border-b border-slate-50 transition-colors hover:bg-primary/5 last:border-0 rounded-lg">
                                    ...
                                    <div x-show="filteredDrivers.length === 0" class="flex gap-2 items-center justify-center px-4 py-4 text-xs font-bold text-slate-500 bg-slate-50 rounded-xl m-1">
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="2" placeholder="اختياري..."
                            class="p-4 w-full text-sm font-bold bg-slate-50/50 rounded-xl border border-slate-200 outline-none resize-none focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 text-slate-700 transition-all">{{ old('notes') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[2rem] border border-slate-100 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)] overflow-visible relative">
                    ...
                    <div class="flex gap-3 items-center">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] font-bold text-slate-400 mt-0.5">
                                العدد الإجمالي: <span class="text-primary font-black ml-1" x-text="items.length"></span>
                            </p>
                        </div>
                    </div>

                    <button type="button" @click="addItem()"
                        class="w-10 h-10 rounded-xl flex items-center justify-center text-white bg-primary shadow-sm shadow-primary/30 active:scale-90 transition-all shrink-0">
                ...
                <div class="p-5 space-y-4">
                    <template x-for="(item, index) in items" :key="index">
                        ...
                        <div :class="errorIndices.includes(index) ? 'border-rose-300 shadow-sm' : (activeItem === index ? 'border-primary/40 shadow-[0_8px_30px_-10px_rgba(var(--color-primary-rgb),0.3)]' : 'border-slate-100 hover:border-slate-200')"
                            ...
                            <div @click="activeItem = activeItem === index ? null : index"
                                class="flex justify-between items-center p-4 cursor-pointer select-none transition-colors"
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <button type="button" @click.stop="removeItem(index)" x-show="activeItem === index && items.length > 1"
                                        class="w-8 h-8 flex items-center justify-center text-rose-400 hover:bg-rose-50 rounded-lg transition-colors active:scale-90">
                                        <span class="material-symbols-outlined text-[18px]">delete</span>
                                    </button>
                                    
                                    {{-- سهم الفتح والإغلاق --}}
                                    <div class="w-8 h-8 flex items-center justify-center rounded-full transition-colors"
                                ...
                                <div class="p-5 space-y-5">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" :name="`items[${index}][bond_number]`" x-model="item.bond_number" placeholder="رقم السند"
                                            class="px-3 w-full h-11 text-xs font-bold bg-slate-50/50 rounded-xl border border-slate-200 outline-none focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 text-slate-700 font-mono">
                                    </div>

                                    {{-- ================= بيانات المرسل (ذكي) ================= --}}
                                    <div class="p-3 bg-slate-50/80 border border-slate-100 rounded-xl space-y-3" x-data="{
                                        ...
                                        <div class="relative">
                                            ...
                                            <div class="flex overflow-visible items-center w-full h-11 bg-white rounded-lg border border-slate-200 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20">
                                                ...
                                                <input type="tel" x-model="sdLocal" @input="onPhoneInput()" @focus="sdOpen = true" @click.outside="sdOpen = false" placeholder="7XXXXXXXX" dir="ltr" autocomplete="off"
                                                    class="flex-grow min-w-0 px-3 w-full h-full text-xs font-bold tracking-wider text-left bg-transparent border-none outline-none focus:ring-0 text-slate-700">

                                                {{-- زر الدولة (على اليسار) --}}
                                                <button type="button" @click="sdOpen = !sdOpen" class="flex gap-1.5 items-center px-2.5 h-full bg-slate-50 rounded-l-lg border-r border-slate-200 shrink-0 transition-colors hover:bg-slate-100">
                                            ...
                                            <div x-show="sdOpen && sdLocal.trim().length > 0" x-transition x-cloak
                                                ...
                                                <template x-for="customer in sdFilteredCustomers" :key="customer.id">
                                                    <button type="button" @click="selectCustomer(customer)"
                                                        class="flex justify-between items-center px-3 py-2.5 w-full text-right border-b border-slate-50 transition-colors hover:bg-primary/5 last:border-0 rounded-lg">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" :name="`items[${index}][sender_name]`" x-model="item.sender_name" placeholder="اختياري"
                                                class="px-3 w-full h-11 text-xs font-bold bg-white rounded-lg border border-slate-200 outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 text-slate-700">
                                        </div>
                                    </div>
                                    ...
                                    <div class="p-3 bg-slate-50/80 border border-slate-100 rounded-xl space-y-3" x-data="{
                                        ...
                                        <div class="relative">
                                            ...
                                            <div class="flex overflow-visible items-center w-full h-11 bg-white rounded-lg border border-slate-200 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20">
                                                ...
                                                {{-- زر الدولة (على اليسار) --}}
                                                <button type="button" @click="rcOpen = !rcOpen" class="flex gap-1.5 items-center px-2.5 h-full bg-slate-50 rounded-l-lg border-r border-slate-200 shrink-0 transition-colors hover:bg-slate-100">
                                    ...
                                    <div class="grid grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <div class="flex p-1 bg-slate-100 rounded-xl border border-slate-200/50">
                                    ...
                                    <div class="grid grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <select :name="`items[${index}][package_type]`" x-model="item.package_type"
                                                class="px-3 w-full h-11 text-xs font-bold bg-slate-50/50 rounded-xl border border-slate-200 outline-none focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 text-slate-700 appearance-none">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" :name="`items[${index}][item_notes]`" x-model="item.item_notes" placeholder="..."
                                                class="px-3 w-full h-11 text-xs font-bold bg-slate-50/50 rounded-xl border border-slate-200 outline-none focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 text-slate-700">
                                        </div>
                                    </div>
                    ...
                    <div x-show="items.length === 0" class="flex flex-col justify-center items-center py-10 bg-slate-50 rounded-2xl border border-slate-200 border-dashed">
