...
@section('content')
    ...
    <div class="flex flex-col pt-4 pb-24 min-h-screen bg-slate-50/50 dark:bg-black">
        ...
        <div class="px-4">
            <div
                class="w-full bg-white dark:bg-boxdark rounded-[2rem] shadow-sm p-5 border border-gray-100 dark:border-boxdark-2">
                ...
                <form action="{{ route('passengers.update', $passenger->id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-5">
                        ...
                        {{-- ================= القسم الأول: التاريخ، المكان، الوجهة، ومكان الراكب ================= --}}
                        <div
                            ...
                            {{-- التاريخ --}}
                            <div>
                                <label class="block mb-2 text-sm font-bold text-gray-600 dark:text-gray-300">تاريخ الرحلة
                                    <span class="text-error">*</span></label>
                                <input type="date" name="date" required
                                    value="{{ $passenger->date ? $passenger->date->format('Y-m-d') : now()->format('Y-m-d') }}"
                            ...
                            {{-- مكان الراكب الحالي --}}
                            <div>
                                 <label class="block mb-2 text-sm font-bold text-gray-600 dark:text-gray-300">مكان 
                                    الراكب <span class="text-error">*</span></label>
                                <input type="text" name="pickup_location" required value="{{ $passenger->pickup_location }}"
                                    placeholder="مثلاً: جولة السفينة"
                                    class="px-4 w-full h-12 rounded-xl border-none ring-1 ring-gray-200 bg-white dark:bg-boxdark dark:text-white focus:ring-2 focus:ring-primary/40 transition-all">
                            </div>

                            {{-- الوجهة --}}
                            <div>
                                <label class="block mb-2 text-sm font-bold text-gray-600 dark:text-gray-300">وجهة الراكب</label>
                                <input type="text" name="destination" value="{{ $passenger->destination }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-1 gap-4">
                            <div class="relative" x-data="passengerPhonePicker(@js(array_values(config('countries', []))))"
                                x-init="loadInitial('{{ $passenger->passenger_number }}')">
                                ...
                                <div class="flex overflow-visible relative items-center bg-white rounded-xl ring-1 ring-gray-200 transition-all dark:bg-boxdark dark:ring-boxdark-2 focus-within:ring-2 focus-within:ring-primary/40"
                                    ...
                                    <div class="relative h-full" @click.away="openCountryDropdown = false">
                                        ...
                                        <div x-show="openCountryDropdown" x-cloak x-transition
                                            ...
                                            <div class="p-2 border-b border-gray-50 dark:border-boxdark"><input type="text"
                                                    x-model="searchCountryQuery" placeholder="بحث..."
                        ...
                        {{-- ================= بيانات السائق ================= --}}
                        {{-- <div
                            class="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 rounded-2xl border border-gray-100 bg-surface dark:bg-boxdark-2 dark:border-boxdark-2"
                            ...
                            <div class="relative">
                                ...
                                <div class="flex overflow-visible relative items-center bg-white rounded-xl ring-1 ring-gray-200 transition-all dark:bg-boxdark dark:ring-boxdark focus-within:ring-2 focus-within:ring-primary/40"
                                    :class="selectedRecordId !== '' ? 'bg-emerald-50/30 dark:bg-emerald-500/10 ring-emerald-400 dark:ring-emerald-500/50' : ''"
                                    style="direction: ltr;">
                                    <div class="relative h-full" @click.away="openCountryDropdown = false">
                                        <button type="button" @click="openCountryDropdown = !openCountryDropdown"
                                ...
                                <div x-show="showDropdown && localPhoneNumber.length > 0 && selectedRecordId === ''"
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="text" name="driver_name" x-model="nameInput"
                                        :readonly="selectedRecordId !== ''" :required="selectedRecordId === ''"
                                    ...
                                    <span
                                        class="absolute right-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-[18px] pointer-events-none"
                                        :class="selectedRecordId !== '' ? 'text-emerald-500' : 'text-gray-400'">local_taxi</span>
                                </div>
                            </div>
                        </div> --}}

                        {{-- ================= الأعداد والعمولات الماليّة ================= --}}
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                <label class="block mb-2 text-sm font-bold text-gray-600 dark:text-gray-300">عدد الركاب
                                    <span class="text-error">*</span></label>
                                <input type="number" name="count" min="1" value="{{ $passenger->count }}" required
                            ...
                            <div>
                                <label class="block mb-2 text-sm font-bold text-gray-600 dark:text-gray-300">عمولة المكتب
                                    <span class="text-error">*</span></label>
                                <input type="number" name="office_commission" min="0" step="0.01" required
                                    placeholder="0.00" value="{{ $passenger->office_commission }}"
                                    class="px-4 w-full h-12 rounded-xl border-none ring-1 ring-gray-200 bg-surface dark:bg-boxdark-2 dark:text-white focus:ring-2 focus:ring-primary/40 text-emerald-600 font-black">
                            </div>
                            <div>
                                <label class="block mb-2 text-sm font-bold text-gray-600 dark:text-gray-300">عمولة مكاتب
                                    أخرى <span class="text-error">*</span></label>
                                <input type="number" name="other_office_commission" min="0" step="0.01" required
                                    placeholder="0.00" value="{{ $passenger->other_office_commission }}"
                        ...
                        <div>
                            ...
                            <textarea name="note" rows="2" placeholder="أي ملاحظات إضافية..."
                                class="px-4 py-3 w-full rounded-xl border-none ring-1 ring-gray-200 bg-surface dark:bg-boxdark-2 dark:text-white focus:ring-2 focus:ring-primary/40">{{ $passenger->note }}</textarea>
                        </div>

                    </div>

                    {{-- زر الحفظ والتأكيد الأساسي --}}
                    <div class="pt-2">
                        <button type="submit"
