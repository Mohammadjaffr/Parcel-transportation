    ...
    <div class="flex flex-col gap-6 px-4 pt-4 pb-24 min-h-screen bg-slate-50/50">
        ...
        <div class="bg-gradient-to-br from-slate-900 to-slate-800 rounded-[2rem] p-6 shadow-lg relative overflow-hidden">
            ...
            <div class="relative z-10 flex justify-between items-start">
                <div>
                    ...
                    <h1 class="text-2xl font-headline font-black text-white mb-1">أهلاً،
                        {{ auth()->user()->name ?? 'محمد' }}</h1>
                    <div
                        class="flex items-center gap-1.5 text-slate-300 text-[10px] font-bold bg-white/10 w-max px-2 py-1 rounded-lg backdrop-blur-sm mt-2">
                        <span class="material-symbols-outlined text-[14px]">storefront</span>
                        {{ auth()->user()->branch->name ?? 'الفرع الرئيسي' }}
                    </div>
                </div>
                <div
                    class="w-12 h-12 rounded-2xl bg-white/10 border border-white/20 flex items-center justify-center backdrop-blur-md shadow-inner">
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-3 mt-2">
                ...
                {{-- 1. عملاء عليهم فلوس (ديون) --}}
                <div
                    class="bg-white rounded-[1.5rem] p-5 border border-slate-100 shadow-sm flex flex-col gap-1 relative overflow-hidden group">
                    <div
                        class="absolute -right-4 -bottom-4 w-16 h-16 bg-rose-50 rounded-full transition-transform group-hover:scale-150">
                    </div>
                    <div class="relative z-10">
                        <div class="flex items-center gap-1.5 text-rose-500 mb-3">
                            <span class="material-symbols-outlined text-[18px]">money_off</span>
                            <span class="text-xs font-bold text-slate-500">عملاء عليهم ديون</span>
                        </div>
                        <span
                            class="text-3xl font-headline font-black text-slate-800">{{ number_format($debtorsCount ?? 0) }}</span>
                    </div>
                </div>

                {{-- 2. عملاء لهم فلوس (أرصدة) --}}
                <div
                    class="bg-white rounded-[1.5rem] p-5 border border-slate-100 shadow-sm flex flex-col gap-1 relative overflow-hidden group">
                    <div
                        class="absolute -right-4 -bottom-4 w-16 h-16 bg-emerald-50 rounded-full transition-transform group-hover:scale-150">
                    </div>
                    <div class="relative z-10">
                        <div class="flex items-center gap-1.5 text-emerald-500 mb-3">
                            <span class="material-symbols-outlined text-[18px]">account_balance_wallet</span>
                            <span class="text-xs font-bold text-slate-500">عملاء لهم أرصدة</span>
                        </div>
                        <span
                            class="text-3xl font-headline font-black text-slate-800">{{ number_format($creditorsCount ?? 0) }}</span>
                    </div>
                </div>

                {{-- 3. تم التسليم (باقي كما هو) --}}
                <div
                    class="bg-white rounded-[1.5rem] p-5 border border-slate-100 shadow-sm flex flex-col gap-1 relative overflow-hidden group">
                    <div
                        class="absolute -right-4 -bottom-4 w-16 h-16 bg-emerald-50 rounded-full transition-transform group-hover:scale-150">
                    ...
                    <div class="relative z-10">
                        ...
                        <span
                            class="text-3xl font-headline font-black text-slate-800">{{ number_format($stats['delivered'] ?? 0) }}</span>
                    </div>
                </div>

                {{-- 4. المرتجعات (باقي كما هو) --}}
                <div
                    class="bg-white rounded-[1.5rem] p-5 border border-slate-100 shadow-sm flex flex-col gap-1 relative overflow-hidden group">
                    <div
                        class="absolute -right-4 -bottom-4 w-16 h-16 bg-rose-100/50 rounded-full transition-transform group-hover:scale-150">
                    </div>
                    <div class="relative z-10">
                        <div class="flex items-center gap-1.5 text-rose-500 mb-3">
                        ...
                        <span
                            class="text-3xl font-headline font-black text-rose-600">{{ number_format($stats['returned'] ?? 0) }}</span>
                    </div>
                </div>
                ...
                <a href="{{ route('passengers.index') }}"
                    ...
                    {{-- تأثيرات الخلفية --}}
                    <div
                        class="absolute -right-4 -bottom-4 w-24 h-24 bg-white/10 rounded-full transition-transform group-hover:scale-150">
                    </div>
                    ...
                    <div class="relative z-10 flex items-center gap-4 text-white">
                        <div
                            class="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center backdrop-blur-sm shrink-0 border border-white/20">
                            <span class="material-symbols-outlined text-[26px]">hail</span>
                        </div>
                        <div>
                            <p class="text-[11px] font-bold text-indigo-100 mb-0.5">إدارة الركاب والرحلات</p>
                            <div class="flex items-end gap-2">
                                <span class="text-3xl font-headline font-black">{{ $pendingPassengersCount ?? 0 }}</span>
                                <span class="text-xs font-bold text-indigo-200 mb-1.5">راكب قيد الانتظار</span>
                            </div>
                        </div>
                    </div>
        ...
        <div>
            <div class="flex justify-between items-center mb-3 px-1">
                ...
                <a href="{{ route('shipment.outgoing.index') }}" class="text-[10px] text-primary hover:underline">عرض
                    الكل</a>
            </div>
            ...
            <div class="space-y-3">
                @forelse($latestShipments as $shipment)
                    ...
                    <a href="{{ route('shipment.outgoing.show', $shipment->id) }}"
                        class="bg-white p-4 rounded-[1.5rem] border border-slate-100 shadow-sm flex items-center justify-between transition-transform active:scale-95 block">
                        <div class="flex items-center gap-3">
                            <div
                                class="w-12 h-12 {{ $statusStyle['bg'] }} rounded-[1rem] flex items-center justify-center {{ $statusStyle['text'] }} shrink-0">
                        ...
                        <span
                            class="px-2.5 py-1 {{ $statusStyle['bg'] }} {{ $statusStyle['text'] }} {{ $statusStyle['border'] }} border rounded-lg text-[9px] font-black shrink-0">
                            {{ $statusStyle['label'] }}
                        </span>
                    </a>
                @empty
                    <div
                        class="py-8 flex flex-col items-center justify-center bg-white rounded-[1.5rem] border border-dashed border-slate-200 text-center">
