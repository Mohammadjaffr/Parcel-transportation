    ...
    <div class="flex flex-col gap-6 px-4 pt-4 pb-24 min-h-screen bg-slate-50/50">
        ...
        <div class="flex justify-between items-center">
            ...
            <a href="https://wa.me/{{ str_starts_with(ltrim($customer->phone, '+'), '7') ? '967' . ltrim($customer->phone, '+') : ltrim($customer->phone, '+') }}" target="_blank"
                ...
                <svg class="w-4 h-4 fill-emerald-500" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path
            ...
            <a href="{{ route('receipt.generate', ['type' => 'CustomerAccountStatementReceipt', 'id' => $customer->uuid]) }}"
                target="_blank"
                class="flex gap-2 items-center px-2 h-10 text-xs font-bold rounded-xl border transition-transform border-pri-100 bg-emeprald-50 text-p-600 active:scale-95">
        ...
        {{-- ================= بطاقة الهوية ================= --}}
        <div
            class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)] flex items-center gap-4 relative overflow-hidden">
            <div
                class="flex justify-center items-center w-16 h-16 text-2xl font-black rounded-2xl border shadow-inner bg-primary/10 text-primary border-primary/20 shrink-0">
                @php
        ...
        <div>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                <div
                    class="col-span-2 bg-white p-5 rounded-[1.5rem] border border-slate-100 shadow-sm flex items-center justify-between">
                    <div>
                        <p class="text-[10px] font-bold text-slate-400 mb-1">إجمالي المبلغ المتبقي عليه</p>
                        <p
                            class="text-2xl font-black font-headline {{ $grandTotalRemaining > 0 ? 'text-rose-500' : 'text-slate-800' }}">
                            {{ number_format($grandTotalRemaining, 0) }} <span class="text-xs text-slate-400">ريال</span>
                        </p>
                    </div>
                    <div
                        class="w-12 h-12 rounded-xl {{ $grandTotalRemaining > 0 ? 'bg-rose-50 text-rose-500' : 'bg-slate-50 text-slate-400' }} flex items-center justify-center">
            ...
            @if ($unpaidShipmentsCount > 0)
                <div
                    class="mt-3 px-4 py-2.5 bg-rose-50/50 border border-rose-100 rounded-xl flex items-center gap-2 text-rose-600 text-[10px] font-bold">
        ...
        <div class="mt-2">
            ...
            <div class="space-y-4">
                    ...
                    <div class="bg-white rounded-[1.5rem] border border-slate-200/60 shadow-sm overflow-hidden relative">
                        ...
                        <div class="flex justify-between items-center px-4 py-2 border-b border-slate-100 bg-slate-50/50">
                            @if ($shipment->sender_customer_id == $customer->id)
                                <span
                                    class="px-2 py-0.5 rounded-md text-[9px] font-black bg-primary/10 text-primary border border-primary/20 flex items-center gap-1">
                                    <span class="material-symbols-outlined text-[12px]">arrow_upward</span>
                                    مرسل
                                </span>
                            @else
                                <span
                                    class="px-2 py-0.5 rounded-md text-[9px] font-black bg-emerald-100 text-emerald-700 border border-emerald-200 flex items-center gap-1">
                            ...
                            <span
                                class="text-[10px] font-bold text-slate-400">{{ $shipment->created_at->format('Y-m-d') }}</span>
                        </div>

                        <div class="flex justify-between items-center p-4">
                            <div>
                                <h3 class="font-mono text-sm font-black tracking-tight text-slate-800">
                                ...
                                <p class="text-[10px] font-bold text-slate-500 mt-1">
                                    ...
                                    @endif
                                    •
                                    <span class="font-black text-slate-800">{{ number_format($shipment->total_amount, 0) }}
                                        ريال</span>
                                </p>
                            </div>

                            <a href="{{ route('shipment.outgoing.show', $shipment->id) }}"
                                class="flex justify-center items-center w-10 h-10 rounded-full border transition-colors bg-slate-50 text-slate-400 hover:bg-primary hover:text-white border-slate-100">
                ...
                @empty
                    <div
                        class="py-12 flex flex-col items-center justify-center bg-white rounded-[2rem] border-2 border-dashed border-slate-100 text-center">
